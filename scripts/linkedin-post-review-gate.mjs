...
const INTERNAL_REFERENCE_PATTERNS = [
  ...
  /(?:^|\n)\s*Source:\s*(?:apps|src|docs|content|frontend|packages|\/)/i,
  /(?:^|\n)\s*Resource:\s*\//i,
...
const UNVERIFIED_LEAD_MAGNET_PATTERNS = [
  /\bno\s+email\s+required\b/i,
  /\bfree\s+download\b.*\bgrantpipe\.com\/(?:free|resources)\//i,
  /\bdownload\s+(?:the|our|this)\b.*\bgrantpipe\.com\/(?:free|resources)\//i,
  /\bgrab\s+(?:the|our)\s+free\b/i,
];

const HARDCODED_GRANTPIPE_PRICING_PATTERNS = [
  /\bGrantPipe\b[\s\S]{0,160}\$\d[\d,]*(?:\/mo|\/month|\s+per\s+month)\b/i,
  /\$\d[\d,]*(?:\/mo|\/month|\s+per\s+month)\b[\s\S]{0,160}\bGrantPipe\b/i,
  /\b(?:Starter|Growth|Audit[- ]Ready)\s+(?:plan\s+)?(?:at|from|is)\s+\$\d[\d,]*(?:\/mo|\/month|\s+per\s+month)\b/i,
  /\b(?:Starter|Growth|Audit[- ]Ready):\s*\$\d[\d,]*(?:\/mo|\/month|\s+per\s+month)\b/i,
  /\$\d[\d,]*\/yr\b/i,
];

const HARDCODED_GRANTPIPE_PROMO_PATTERNS = [
  /\bLimited offer:\s*80%\s+off\s+(?:your|the)\s+first\s+year\b/i,
  /\bFirst\s+300\s+customers\b/i,
];
...
export function reviewLinkedInPost({
  id,
  content,
  attachments = [],
  metadata = {},
  ...
  for (const pattern of MISSING_MEDIA_PATTERNS) {
    if (pattern.test(normalized)) {
      const isProductionInstruction =
        /suggestion|idea|prompt|description|use|create|insert|add/i.test(pattern.source);
  ...
  for (const pattern of UNVERIFIED_LEAD_MAGNET_PATTERNS) {
    if (pattern.test(normalized)) {
      errors.push(`contains unverified lead-magnet/download CTA matching ${pattern}`);
    }
  }

  for (const pattern of HARDCODED_GRANTPIPE_PRICING_PATTERNS) {
    if (pattern.test(normalized)) {
      errors.push(`contains hardcoded GrantPipe pricing matching ${pattern}`);
    }
  }

  for (const pattern of HARDCODED_GRANTPIPE_PROMO_PATTERNS) {
    if (pattern.test(normalized)) {
      errors.push(`contains hardcoded GrantPipe promo terms matching ${pattern}`);
    }
  }
  ...
  if (reviewStatus === undefined) {
    if (requireMetadata) {
      errors.push("missing review_status");
    }
  } else if (
    !/(passed|approved|reviewed|ready[_ -]?to[_ -]?schedule)/i.test(String(reviewStatus))
  ) {
    errors.push(`review_status is ${reviewStatus}`);
  }

  const humanizerStatus = metadata?.humanizer_status ?? metadata?.humanizerStatus;
  if (humanizerStatus === undefined) {
    if (requireMetadata) {
      errors.push("missing humanizer_status");
    }
  } else if (String(humanizerStatus) !== "passed") {
    errors.push(`humanizer_status is ${humanizerStatus}`);
  }

  const claimSources = metadata?.claim_sources ?? metadata?.claimSources;
  if (claimSources === undefined) {
    if (requireMetadata || requireClaimSources) {
      errors.push("missing claim_sources");
    }
  } else if (!Array.isArray(claimSources) || claimSources.length === 0) {
    errors.push("claim_sources must be a non-empty array");
  }
...
function pickMetadataFields(item) {
...
function postsFromJson(value, source) {
  ...
  return items.flatMap((item, index) => {
    ...
    const requireMetadata =
      item.postsAndComments !== undefined || item.posts !== undefined || item.value !== undefined;
    return [
      { id, content, attachments, metadata: pickMetadataFields(item), requireMetadata, source },
    ];
  });
}

function readCsvCell(row, idx, name) {
  const index = idx[name];
  return index === undefined ? undefined : row[index];
}

function parseClaimSources(value) {
...
function metadataFromCsvRow(row, idx) {
  const metadata = {};
  for (const [csvName, metadataName] of [
    ["review_status", "review_status"],
    ["reviewStatus", "reviewStatus"],
    ["humanizer_status", "humanizer_status"],
    ["humanizerStatus", "humanizerStatus"],
  ]) {
    const value = readCsvCell(row, idx, csvName);
    if (value !== undefined) {
      metadata[metadataName] = value;
    }
  }

  const claimSources = parseClaimSources(
    readCsvCell(row, idx, "claim_sources") ?? readCsvCell(row, idx, "claimSources"),
  );
  if (claimSources !== undefined) {
    metadata.claim_sources = claimSources;
  }
...
function postsFromCsv(text, source) {
  ...
  return rows.map((row, index) => ({
    ...
    content: row[idx.post_text] || row[idx.post_content] || row[idx.content] || row[idx.text] || "",
    attachments: [],
    metadata: metadataFromCsvRow(row, idx),
    source,
  }));
}
...
function main() {
  ...
  const files = targets
    ...
    .filter((path) =>
      [".txt", ".md", ".json", ".jsonl", ".csv"].includes(extname(path).toLowerCase()),
    );
