...
- `packages/db/src/client.ts` currently selects `@neondatabase/serverless` for non-local, non-Hyperdrive database URLs and `pg.Pool` only for local or Hyperdrive connections.
- `apps/api/src/app.ts` intentionally bypasses Hyperdrive for scheduled jobs because Neon cold-wake behavior required the WebSocket serverless driver.
- `apps/api/src/lib/db-retry.ts`, `apps/api/src/app.test.ts`, docs, and comments contain Neon-specific error names and provider assumptions.
...
- Modify: `CLAUDE.md`, `AGENTS.md`, `docs/architecture/third-party-dependency-map.md`, `docs/go-live-manual.md`, `docs/production-readiness.md`, `docs/production-e2e-cleanup.md`, and `apps/site/src/pages/privacy.astro` - update provider docs and subprocessor copy.
...
- [ ] **Step 1: Point local app processes at the Supabase rehearsal DB**
...
- [ ] **Step 2: Apply migrations against the rehearsal Supabase DB**
...
- [ ] **Step 3: Build all deployable apps locally**
...
- [ ] **Step 4: Run local browser E2E across core flows**
...
- [ ] **Step 5: Run local production-stress harness contract tests**
...
- [ ] **Step 6: Run local cleanup dry-run against rehearsal DB**
