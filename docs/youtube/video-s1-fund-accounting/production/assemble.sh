...
echo "joining with ${XF}s cross-dissolves -> fund-accounting.mp4"
ffmpeg -y -loglevel error "${INPUTS[@]}" \
  -filter_complex "$FILTER" \
  -map "$prev_v" -map "$prev_a" \
  -c:v libx264 -preset medium -crf 19 -pix_fmt yuv420p \
  -c:a aac -b:a 192k -ar 48000 -ac 2 -movflags +faststart \
  "$OUT/fund-accounting.mp4"

echo "DONE"
ffprobe -v error -show_entries format=duration:stream=codec_type,width,height -of default=noprint_wrappers=1 "$OUT/fund-accounting.mp4"
