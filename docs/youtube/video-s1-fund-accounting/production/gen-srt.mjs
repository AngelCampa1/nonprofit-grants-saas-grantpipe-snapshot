...
for (let ci = 0; ci < chapText.length; ci++) {
  ...
  // The next chapter's first cue starts XFADE earlier than this chapter's nominal
  // end (start+duration), because the crossfade pulls every later chapter back by
  // (ci+1)*XFADE. Clamp this chapter's cue ends to that boundary so the last cue
  // never double-displays into the next chapter's opening line across the seam.
  const nextAStart =
    ci + 1 < chapText.length ? durations[ci + 1].start - (ci + 1) * XFADE : Infinity;
  const capEnd = Math.min(aStart + duration, nextAStart);
