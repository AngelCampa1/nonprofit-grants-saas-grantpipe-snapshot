...
OUT=../output
AUD=../audio
CHAPTERS=(00 01 02 03 04 05 06 07 08 09)
...
echo "concatenating -> grant-tracking-spreadsheet.mp4"
ffmpeg -y -loglevel error -f concat -safe 0 -i "$LIST" \
  -c:v libx264 -preset medium -crf 19 -pix_fmt yuv420p \
  -c:a aac -b:a 192k -ar 48000 -movflags +faststart \
  "$OUT/grant-tracking-spreadsheet.mp4"

echo "DONE"
ffprobe -v error -show_entries format=duration:stream=codec_type,width,height -of default=noprint_wrappers=1 "$OUT/grant-tracking-spreadsheet.mp4"
