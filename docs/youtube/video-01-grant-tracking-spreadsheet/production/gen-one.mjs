...
const VOICE_SETTINGS = { stability: 0.45, similarity_boost: 0.8, style: 0.15, use_speaker_boost: true, speed: 0.97 };

const key = readFileSync(ENV_PATH, "utf8").match(/^ELEVENLABS_API_KEY=(.+)$/m)[1].trim();
const [, , out, text] = process.argv;
if (!out || !text) { console.error("usage: node gen-one.mjs <out.mp3> <text>"); process.exit(1); }
...
if (!res.ok) { console.error(`ElevenLabs ${res.status}: ${(await res.text()).slice(0,400)}`); process.exit(1); }
