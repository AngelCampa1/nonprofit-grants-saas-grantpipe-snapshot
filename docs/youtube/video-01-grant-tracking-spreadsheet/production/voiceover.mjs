...
function parseChapters(md) {
  ...
  for (const raw of lines) {
    ...
    if (s.startsWith("### ")) {
      ...
      const slug = title.toLowerCase().replace(/\[[^\]]*\]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 40);
...
for (const c of chapters) {
  ...
  manifest.push({ id: c.id, title: c.title, slug: c.slug, chars: c.text.length, file: `audio/chapter-${c.id}.mp3`, text: c.text });
}
