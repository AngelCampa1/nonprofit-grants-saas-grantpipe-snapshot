...
const isCue = (s) => s.startsWith("#") || s.startsWith("[VISUAL") || s.startsWith(">") || s.startsWith("**") || s.startsWith("---");
const chapters = [];
let cur = null;
for (const raw of md.split("\n")) {
  const s = raw.trim();
  if (s.startsWith("### ")) { cur = { narration: [] }; chapters.push(cur); continue; }
  if (!cur || !s || isCue(s)) continue;
  cur.narration.push(s);
}
const chapText = chapters.map((c) => c.narration.join(" ").replace(/\s+/g, " ").trim()).filter(Boolean);
  ...
  for (let p of parts) {
    ...
    if (p.length > 110 && p.includes(",")) {
      ...
      for (const seg of sub) {
        if ((buf + ", " + seg).length > 90 && buf) { out.push(buf.trim()); buf = seg; }
...
for (let ci = 0; ci < chapText.length; ci++) {
  ...
  for (const sent of sents) {
    ...
    const end = t + dur;
    lines.push(`${idx}`);
    lines.push(`${fmt(t)} --> ${fmt(Math.min(end, start + duration))}`);
    lines.push(sent);
    lines.push("");
    idx++;
    t = end;
