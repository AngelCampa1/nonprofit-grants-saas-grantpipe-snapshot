...
export type PrimaryCta = "trial" | "lead-magnet" | "compare" | "pricing" | "contact";
...
/**
 * Promotional banner shown above the sticky site header on every marketing page.
 * Optional and time-bound: when `endsAt` has passed or the user dismisses it,
 * the banner self-hides via an inline script. Leave the field unset on
 * `SiteConfig` to render no banner.
 */
export interface PromoBannerConfig {
  /** Short uppercase eyebrow shown before the message (e.g. "Limited-time offer"). */
  eyebrow?: string;
  /** Main banner copy. Required - the banner does not render without it. */
  message: string;
  /** Optional CTA label rendered as an inline link/button. */
  ctaText?: string;
  /** Optional CTA destination. Used together with `ctaText`. */
  ctaHref?: string;
  /** ISO timestamp after which the banner self-hides client-side. */
  endsAt?: string;
  /** Show the dismiss control. Defaults to true. */
  dismissible?: boolean;
  /** localStorage key used to persist dismissals across sessions. */
  storageKey?: string;
  /** Accessible label for the banner region. Defaults to "Promotional announcement". */
  ariaLabel?: string;
}
...
export interface LeadMagnetOffer {
  ...
  description: string;
  slug?: LeadMagnetSlug;
  alternatives?: Array<Omit<LeadMagnetOffer, "alternatives"> & { slug: LeadMagnetSlug }>;
  teaser?: string;
  ctaText?: string;
  headline?: string;
...
export interface LeadMagnetConfig {
  featuredSlugs: LeadMagnetSlug[];
  fallbackByFamily: Record<LeadMagnetRouteFamily, LeadMagnetSlug>;
}
...
export type TrustSignalCategory = "feature" | "roi" | "compliance" | "integration";
...
export interface SiteConfig {
  ...
  copy?: {
    ...
    survey?: {
      ...
      /** Override URL for the qualified CTA. Defaults to siteConfig.discoveryCallUrl when absent. */
...
export interface PricingTier {
  ...
  annualPriceCents?: number; // actual annual total in cents; overrides monthlyPriceCents * 10 formula
  annualPriceOverride?: string; // optional custom annual display, e.g. "$24.99/yr"
  monthlyPromoPrice?: string;
  annualPromoPrice?: string;
  promoBadge?: string;
  pricingModel?: PricingModel; // defaults to "flat" if omitted
  unitLabel?: string; // e.g. "/user", "/child" - appended to computed annual prices
  description?: string;
  bestFit?: string; // short "Best for X" line shown in pricing cards
  features: string[];
