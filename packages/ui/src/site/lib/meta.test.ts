...
describe("ensureTrailingSlash", () => {
  it("appends slash when URL has no trailing slash", () => {
    expect(ensureTrailingSlash("https://example.com/foo")).toBe("https://example.com/foo/");
  });
...
describe("truncateMetaTitle", () => {
  ...
  it("truncates at word boundary when too long", () => {
    ...
    expect(result).toBe("Alpha beta gamma delta epsilon zeta eta theta iota kappa...");
  });

  it("keeps the full last whole word before the suffix", () => {
    expect(truncateMetaTitle("Alpha beta gamma delta epsilon", 25)).toBe(
...
describe("truncateMetaDescription", () => {
  ...
  it("truncates at word boundary when too long", () => {
    ...
    expect(result.endsWith("...")).toBe(true);
    expect(result).toBe(
      "Arizona childcare software guide for licensed centers regulated by ADHS Bureau of Child Care Licensing under Arizona Administrative Code A.A.C. R9-5 with...",
    );
  });

  it("does not trim extra characters before the suffix", () => {
    expect(truncateMetaDescription("Alpha beta gamma delta epsilon zeta", 30)).toBe(
      "Alpha beta gamma delta...",
    );
  });

  it("backs up past dangling conjunctions before adding the suffix", () => {
    expect(truncateMetaDescription("Alpha beta gamma and epsilon", 25)).toBe("Alpha beta gamma...");
  });
...
describe("resolveSchemaImage", () => {
  ...
  it("prepends https: to protocol-relative ogImage", () => {
    expect(resolveSchemaImage("example.com", "//cdn.com/img.png")).toBe("https://cdn.com/img.png");
  });

  it("prepends domain to root-relative ogImage", () => {
    expect(resolveSchemaImage("example.com", "/img.png")).toBe("https://example.com/img.png");
  });
  ...
  it("falls back to og-default.png when both are undefined", () => {
    expect(resolveSchemaImage("example.com")).toBe("https://example.com/og-default.png");
  });
});
...
describe("resolveOgImage", () => {
  it("returns root-relative ogImage with siteUrl prepended", () => {
    expect(resolveOgImage("https://example.com/page", "/og.png", "https://example.com")).toBe(
  ...
  it("returns ogImage as-is when it is not root-relative", () => {
    expect(
      resolveOgImage("https://example.com/page", "https://cdn.com/og.png", "https://example.com"),
    ).toBe("https://cdn.com/og.png");
  });

  it("returns ogImage as-is when root-relative but no siteUrl", () => {
    expect(resolveOgImage("https://example.com/page", "/og.png")).toBe("/og.png");
  });

  it("returns protocol-relative ogImage as-is", () => {
    expect(
      resolveOgImage("https://example.com/page", "//cdn.com/og.png", "https://example.com"),
    ).toBe("//cdn.com/og.png");
  });

  it("falls back to origin + /og-default.png when no ogImage", () => {
    expect(resolveOgImage("https://example.com/page")).toBe("https://example.com/og-default.png");
  });
