 ...
 * Rounds up to whole dollars so public monthly-equivalent prices never show cents.
 */
export function formatAnnualMonthlyEquivalent(
  ...
  const dollars = Math.ceil(monthlyEquivalentCents / 100);
  const formatted = formatDollars(dollars);
  const unit = unitLabel ?? "";
...
/**
 * Given an actual annual price in cents, format it as "$X,XXX/yr".
 * Use this when the annual total is not derived from the monthly × 10 formula
 * (e.g. Stripe prices with custom annual amounts).
 */
export function formatAnnualPriceFromCents(annualCents: number, unitLabel?: string): string {
  const dollars = annualCents / 100;
  const formatted = formatDollars(dollars);
  const unit = unitLabel ?? "";
  return `$${formatted}${unit}/yr`;
...
/**
 * Given an actual annual price in cents, compute the per-month equivalent.
 * (annualCents / 12) formatted as "~$XX/mo".
 * Use this when the annual total is not derived from the monthly × 10 formula.
 */
export function formatAnnualMonthlyEquivalentFromCents(
  annualCents: number,
  unitLabel?: string,
): string {
  const monthlyEquivalentCents = annualCents / 12;
