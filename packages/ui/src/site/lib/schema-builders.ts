...
export function buildBreadcrumbSchema(items: BreadcrumbItemInput[], siteUrl: string): JsonLdSchema {
  return {
    ...
    itemListElement: items.map((item, index) => ({
      ...
      position: index + 1,
      name: item.label,
      item: joinUrl(siteUrl, item.href),
    })),
  };
}
...
export function buildOrganizationSchema(opts: OrganizationSchemaOpts): JsonLdSchema {
...
export function buildSoftwareApplicationSchema(opts: SoftwareApplicationSchemaOpts): JsonLdSchema {
  const {
    name,
    description,
    offers,
...
export function buildGeoArticleSchema(opts: GeoArticleSchemaOpts): JsonLdSchema {
 ...
 * this source".
 */
export function buildStatisticCitationSchema(opts: StatisticCitationSchemaOpts): JsonLdSchema {
...
export function buildPriceSpecificationSchema(tiers: PriceTierInput[]): JsonLdSchema {
...
export function buildCollectionPageSchema(opts: CollectionPageSchemaOpts): JsonLdSchema {
