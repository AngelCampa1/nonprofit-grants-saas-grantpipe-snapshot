...
describe("splitContentAtGate", () => {
  ...
  it("splits at the Nth h2 boundary", () => {
    ...
    expect(result.teaser).toBe("<h2>Section 1</h2><p>Content for section 1.</p>");
  ...
  it("splits at the 2nd h2 boundary", () => {
    ...
    expect(result.gated).toBe("<h2>Section 3</h2><p>Content for section 3.</p>");
  });
  ...
  it("falls back to paragraph splitting when there are no headings", () => {
    const noHeadings = "<p>Paragraph 1.</p><p>Paragraph 2.</p><p>Paragraph 3.</p>";
  ...
  it("falls back to h3 when there are no h2 headings but h3 exist", () => {
    ...
    expect(result.gated).toBe("<h3>Sub 2</h3><p>Content 2.</p><h3>Sub 3</h3><p>Content 3.</p>");
  });
  ...
  it("splits on h2 even when h3 headings are also present", () => {
    ...
    expect(result.teaser).toBe("<h2>Section 1</h2><h3>Sub A</h3><p>Content.</p>");
  ...
  it("handles content with leading text before first heading", () => {
    ...
    expect(result.teaser).toBe("<p>Intro text.</p><h2>Section 1</h2><p>Body.</p>");
