...
describe("generateThemeCSS", () => {
  describe("GrantPipe marketing redesign handoff tokens", () => {
    it("aliases every --gp-* handoff token to a canonical design-system token", () => {
      ...
      expect(css).toContain("--site-primary: #065f46;");
      expect(css).toContain("--site-accent: #b88928;");
      expect(css).toContain("--site-surface: #fbfaf6;");
      expect(css).toContain("--site-text: #0e1a16;");
      expect(css).toContain("--site-muted: #5b6e66;");

      // Color aliases stay wired to canonical scales.
      expect(css).toContain("--gp-emerald-900: var(--color-primary-950);");
      expect(css).toContain("--gp-emerald-700: var(--color-primary-800);");
      expect(css).toContain("--gp-emerald-50: var(--color-primary-50);");
      expect(css).toContain("--gp-gold-800: var(--color-accent-800);");
      expect(css).toContain("--gp-gold-600: var(--color-accent-600);");
      expect(css).toContain("--gp-gold-50: var(--color-accent-50);");
      expect(css).toContain("--gp-ink-900: var(--color-brand-text);");
      expect(css).toContain("--gp-ink-600: var(--color-neutral-600);");
      expect(css).toContain("--gp-ink-400: var(--color-neutral-400);");
      expect(css).toContain("--gp-ink-100: var(--color-neutral-100);");

      // Surface aliases.
      expect(css).toContain("--gp-paper: var(--surface-primary);");
      expect(css).toContain("--gp-paper-1: var(--surface-primary);");
      expect(css).toContain("--gp-paper-2: var(--surface-secondary);");
      expect(css).toContain("--gp-card: var(--surface-elevated);");
      expect(css).toContain("--gp-danger: var(--color-error-600);");
      expect(css).toContain("--gp-warn: var(--color-accent-600);");

      // Radius aliases into the 5-value scale.
      expect(css).toContain("--gp-rad-sm: var(--radius-xs);");
      expect(css).toContain("--gp-rad: var(--radius-sm);");
      expect(css).toContain("--gp-rad-md: var(--radius-md);");
      expect(css).toContain("--gp-rad-lg: var(--radius-md);");
      expect(css).toContain("--gp-pill: var(--radius-full);");

      // Shadow aliases into the semantic shadow scale.
      expect(css).toContain("--gp-shadow-sm: var(--shadow-sm);");
      expect(css).toContain("--gp-shadow: var(--shadow-card);");
      expect(css).toContain("--gp-shadow-lg: var(--shadow-lg);");
    });

    it("never resolves a --gp-* token to a raw hex literal", () => {
      const themes: Theme[] = [baseTheme, themeWithSurface, horivaTheme];
      for (const theme of themes) {
        const css = generateThemeCSS(theme);
        const gpDeclarations = [...css.matchAll(/(--gp-[a-z0-9-]+):\s*([^;]+);/gi)];
        // The generator must emit at least the full --gp-* handoff block.
        expect(gpDeclarations.length).toBeGreaterThan(20);
        for (const [, token, value] of gpDeclarations) {
          expect(value, `${token} must alias a canonical var(), not a literal`).toMatch(
            /^var\(--[a-z0-9-]+\)$/i,
          );
        }
      }
  ...
  describe("presentation variants", () => {
    ...
    it("uses soft CTA treatment when ctaStyle is soft", () => {
      ...
      expect(css).toContain("--site-primary-button-radius: var(--radius-md);");
    });
  ...
  describe("Horiva palette preservation", () => {
    ...
    it("does not emit an alternate appearance block for tinted editorial themes", () => {
      const css = generateThemeCSS(horivaTheme);
