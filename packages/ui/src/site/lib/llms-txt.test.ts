...
describe("buildLlmsTxt", () => {
        ...
        items: [
          {
            ...
            description: "Side-by-side comparison of CrewRoute and ServiceTitan",
          },
        ],
      },
    ],
  ...
  it("includes the overview paragraph", () => {
    const result = buildLlmsTxt(baseOpts);
    expect(result).toContain("CrewRoute helps owner-operators manage dispatch and scheduling.");
  });

  it("renders optional product fact, best-for, and not-for blocks before sections", () => {
    const result = buildLlmsTxt({
      ...baseOpts,
      facts: ["Pricing: $199-$799/month", "Best for: Mid-sized nonprofits with active grants"],
      bestFor: [
        "Nonprofits that need donors and grants in one system",
        "Teams replacing spreadsheets and consultant-heavy CRM setups",
      ],
      notFor: [
        "Organizations that only need a donation form",
        "Teams that want custom Salesforce development",
      ],
    });

    expect(result).toContain("## Product Facts");
    expect(result).toContain("- Pricing: $199-$799/month");
    expect(result).toContain("## Best For");
    expect(result).toContain("- Nonprofits that need donors and grants in one system");
    expect(result).toContain("## Not For");
    expect(result).toContain("- Organizations that only need a donation form");
    expect(result.indexOf("## Product Facts")).toBeLessThan(result.indexOf("## Alternatives"));
  });
  ...
  it("renders multiple sections with multiple items", () => {
    ...
    expect(result).toContain("- [X vs Y](https://test.com/x-vs-y): Compare X and Y");
  });
  ...
  it("handles all sections being empty", () => {
    ...
    const expected = ["# Empty", "", "> Nothing here", "", "Some overview.", ""].join("\n");
    expect(buildLlmsTxt(opts)).toBe(expected);
  });
...
describe("buildLlmsTxtSections", () => {
  ...
  it("normalizes static paths that do not start with a leading slash", () => {
    const sections = buildLlmsTxtSections("https://example.com/", [
      {
        heading: "Guides",
        entries,
    ...
    expect(sections[0]?.items).toEqual([
      {
        title: "Guide A",
        url: "https://example.com/resources/guides/",
        description: "First guide",
      },
      {
        title: "Guide B",
  ...
  it("normalizes markdown content paths to canonical trailing-slash URLs", () => {
    const sections = buildLlmsTxtSections("https://example.com", [
      {
        heading: "Guides",
        entries: [{ slug: "nonprofit-grant-compliance-guide" }],
        path: (entry) => `/resources/guides/${entry.slug}.md`,
        title: () => "Guide",
    ...
    expect(sections[0]?.items).toEqual([
      {
        title: "Guide",
        url: "https://example.com/resources/guides/nonprofit-grant-compliance-guide/",
        description: "Description",
      },
    ]);
  });
  ...
  it("throws when a manifest entry has no data.title and no explicit title resolver", () => {
    expect(() =>
      buildLlmsTxtSections(siteUrl, [
        {
          heading: "Broken",
          entries: [{ slug: "missing-title" }],
          path: (entry) => `/guides/${entry.slug}`,
          description: () => "Fallback description",
        },
      ]),
    ).toThrow("Llms manifest entries without data.title require an explicit title resolver.");
  });

  it("throws when a manifest entry has no data.description and no explicit description resolver", () => {
    expect(() =>
      buildLlmsTxtSections(siteUrl, [
        {
          heading: "Broken",
          entries: [{ slug: "missing-description" }],
          path: (entry) => `/guides/${entry.slug}`,
