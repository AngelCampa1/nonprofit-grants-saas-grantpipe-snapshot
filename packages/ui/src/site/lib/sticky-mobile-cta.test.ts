...
describe("getStickyMobileCtaRenderState", () => {
  ...
  it("keeps the link but enables interception in pricing-modal mode", () => {
    expect(getStickyMobileCtaRenderState("pricing-modal")).toEqual({
      action: "pricing-modal",
      showLink: true,
  ...
  it("returns no spacer height when the sticky CTA is hidden", () => {
    expect(getStickyMobileCtaSpacerHeight({ isVisible: false, barHeight: 72 })).toBe(0);
  });

  it("returns the sticky CTA height as spacer height when visible", () => {
    expect(getStickyMobileCtaSpacerHeight({ isVisible: true, barHeight: 72 })).toBe(72);
  });
});
