...
describe("initMobileNav", () => {
  ...
  it("tracks when the mobile nav opens and closes", () => {
    const { details, trigger } = buildMobileNav();
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("mobile_nav_opened", {
      page_path: "/pricing",
    });
    expect(mockTrackEvent).toHaveBeenCalledWith("mobile_nav_closed", {
      close_method: "trigger",
      page_path: "/pricing",
    });
  });

  it("tracks clicked mobile nav links with a sanitized destination path", () => {
    const { details, firstLink, trigger } = buildMobileNav();
    firstLink.href = "/first?token=secret&ref=nav";
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("mobile_nav_link_clicked", {
      nav_item_id: "mobile-nav-0",
  ...
  it("falls back to stripping query strings when mobile nav link href parsing fails", () => {
    const { details, firstLink, trigger } = buildMobileNav();
    Object.defineProperty(firstLink, "href", {
  ...
  it("buckets non-http mobile nav href schemes without emitting addresses", () => {
    const { details, firstLink, trigger } = buildMobileNav();
    firstLink.href = "mailto:support@grantpipe.com";
  ...
  it("falls back to the mobile CTA when the nav opens without primary links", () => {
    ...
    details.querySelectorAll("nav[aria-label='Mobile navigation'] > a").forEach((link) => {
      link.remove();
    });
  ...
  it("closes the nav when the mobile CTA is clicked", () => {
    ...
    ctaLink.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true }));
