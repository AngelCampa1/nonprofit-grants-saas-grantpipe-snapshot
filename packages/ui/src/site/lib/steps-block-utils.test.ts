...
describe("normalizeSteps", () => {
  ...
  it("normalizes a single valid step", () => {
    const result = normalizeSteps([{ title: "Step One", content: "Do this first." }]);
    expect(result).toEqual<StepItem[]>([{ title: "Step One", content: "Do this first." }]);
  });
  ...
  it("trims whitespace from title", () => {
    const result = normalizeSteps([{ title: "  Trimmed  ", content: "content" }]);
  ...
  it("preserves content with internal whitespace", () => {
    const result = normalizeSteps([{ title: "T", content: "line one\nline two" }]);
