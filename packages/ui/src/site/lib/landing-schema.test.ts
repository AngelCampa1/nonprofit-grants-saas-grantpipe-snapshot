...
describe("buildLandingSoftwareApplicationProps", () => {
  ...
  it("allows pages without a pricing section to supply a custom offer URL", () => {
    const result = buildLandingSoftwareApplicationProps(baseConfig, {
      canonicalUrl: "https://kaiplan.app/grant-tracking-software/",
      imageUrl: "https://kaiplan.app/og-default.png",
      offersUrl: "https://kaiplan.app/grant-tracking-software/",
    });

    expect(result.offers).toEqual({
      price: "$20/mo",
      url: "https://kaiplan.app/grant-tracking-software/",
    });
  });
});
