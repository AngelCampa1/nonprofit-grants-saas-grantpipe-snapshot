...
export function sortByUpdatedAtDesc<T extends { data: { updatedAt: string } }>(entries: T[]): T[] {
  return [...entries].sort(
...
export function resolveCanonicalUrl(domain: string, basePath: string, currentPage: number): string {
...
export { formatNumber } from "@grantpipe/shared";
