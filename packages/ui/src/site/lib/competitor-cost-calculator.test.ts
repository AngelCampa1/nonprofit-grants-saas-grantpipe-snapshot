...
const COMPETITORS: CompetitorPricing[] = [
  {
    ...
    maxTechs: null,
    isFlatRate: false,
    pricingNote: "~$298/tech/mo (midpoint of $245–$398). $10K setup fee (midpoint of $5K–$50K).",
  },
  {
    slug: "housecall-pro",
    name: "Housecall Pro",
  ...
  },
  {
    ...
    maxTechs: null,
    isFlatRate: false,
    pricingNote: "Core $49/mo (1 user), Connect $149/mo (2–5 users), Grow $399/mo (6+ users).",
  ...
  },
  {
    ...
    maxTechs: 15,
    isFlatRate: true,
    pricingNote: "Solo $79/mo (1 user), Crew $149/mo (2–5 users), Growth $249/mo (6–15 users).",
    calculateMonthly: (teamSize: number) => {
...
describe("generateComparison", () => {
  it("throws RangeError for teamSize=0", () => {
    expect(() => generateComparison(0, COMPETITORS, "crewroute")).toThrow(RangeError);
  });

  it("throws RangeError for teamSize=16", () => {
    expect(() => generateComparison(16, COMPETITORS, "crewroute")).toThrow(RangeError);
  });

  it("throws RangeError for teamSize=-1", () => {
    expect(() => generateComparison(-1, COMPETITORS, "crewroute")).toThrow(RangeError);
  });

  it("throws RangeError for teamSize=20", () => {
    expect(() => generateComparison(20, COMPETITORS, "crewroute")).toThrow(RangeError);
  });
  ...
  describe("teamSize=1", () => {
    it("Jobber is cheapest at teamSize=1 ($49 vs CrewRoute $79)", () => {
      const result = generateComparison(1, COMPETITORS, "crewroute");
      const jobber = result.results.find((r) => r.competitor.slug === "jobber")!;
    ...
    it("savingsVsProduct is positive for competitors more expensive than product", () => {
      const result = generateComparison(1, COMPETITORS, "crewroute");
      const crewroute = result.results.find((r) => r.competitor.slug === "crewroute")!;
      expect(crewroute.savingsVsProduct).toBe(0);

      const servicetitan = result.results.find((r) => r.competitor.slug === "servicetitan")!;
  ...
  describe("firstYearTotal calculation", () => {
    it("firstYearTotal = 12 * monthly + setupFee for servicetitan", () => {
      const result = generateComparison(1, COMPETITORS, "crewroute");
      const st = result.results.find((r) => r.competitor.slug === "servicetitan")!;
      expect(st.firstYearTotal).toBe(12 * st.monthlyTotal + 10000);
    });

    it("firstYearTotal = 12 * monthly for housecall-pro (no setup fee)", () => {
      const result = generateComparison(1, COMPETITORS, "crewroute");
      const hcp = result.results.find((r) => r.competitor.slug === "housecall-pro")!;
  ...
  describe("isCheapest", () => {
    ...
    it("the cheapest result has the lowest monthlyTotal", () => {
      for (let size = 1; size <= 15; size++) {
        const result = generateComparison(size, COMPETITORS, "crewroute");
        const minMonthly = Math.min(...result.results.map((r) => r.monthlyTotal));
        const cheapest = result.results.filter((r) => r.isCheapest);
  ...
  describe("productSlug parameterization", () => {
    ...
    it("throws if productSlug is not found in competitors", () => {
      expect(() => generateComparison(3, COMPETITORS, "nonexistent")).toThrow("not found");
    });
  });
