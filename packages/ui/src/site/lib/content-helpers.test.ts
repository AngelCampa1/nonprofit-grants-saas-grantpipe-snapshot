...
import { STAGE_BADGES, formatContentDate, filterMetadata } from "./content-helpers";
...
describe("STAGE_BADGES", () => {
  ...
  it("has all three buyer stages as keys", () => {
    expect(Object.keys(STAGE_BADGES)).toEqual(expect.arrayContaining(["tofu", "mofu", "bofu"]));
...
describe("filterMetadata", () => {
  ...
  it("excludes keys in excludeKeys list", () => {
    expect(filterMetadata({ a: "x", readTime: "5 min", c: "y" }, ["readTime"])).toEqual([
      ["a", "x"],
      ["c", "y"],
    ]);
  });
