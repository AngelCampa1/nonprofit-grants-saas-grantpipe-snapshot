...
describe("formatSlugAsLabel", () => {
  it("converts a hyphenated slug to title case", () => {
    expect(formatSlugAsLabel("best-apps-make-friends-adult")).toBe("Best Apps Make Friends Adult");
  });
  ...
  it("normalises mixed-case slugs to title case", () => {
    expect(formatSlugAsLabel("HVAC-dispatch-SOFTWARE")).toBe("Hvac Dispatch Software");
  });
