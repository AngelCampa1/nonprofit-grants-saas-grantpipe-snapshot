...
import { indexNowIntegration } from "./indexnow-integration.js";
...
describe("indexNowIntegration", () => {
  ...
  let submitSpy: MockInstance<
    (payload: IndexNowPayload, fetchFn?: typeof fetch) => Promise<IndexNowResult>
  >;

  beforeEach(() => {
    testDir = join(tmpdir(), `indexnow-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  async function invokeHook(overrideDir?: URL) {
    const integration = indexNowIntegration({ enabled: true });
  ...
  it("skips submission entirely when disabled", async () => {
  ...
  it("calls logger.warn when sitemap-index.xml has no child sitemaps", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith("IndexNow: no sitemaps found in sitemap-index.xml");
    expect(submitSpy).not.toHaveBeenCalled();
  });
  ...
  it("logs the URL count and host before submitting", async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith("IndexNow: submitting 2 URLs for crewroute.app");
  });
  ...
  it("logs success message when submission succeeds (status 200)", async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith("IndexNow: submitted successfully (200)");
  });
  ...
  it("logs warning but continues when a child sitemap file can't be read", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("IndexNow: could not read"));
    // No URLs collected → submitToIndexNow should NOT be called (falls into "no URLs" branch)
    expect(logger.warn).toHaveBeenCalledWith("IndexNow: no URLs found in sitemaps");
    expect(submitSpy).not.toHaveBeenCalled();
  });
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <sitemap><loc>https://crewroute.app/sitemap-1.xml</loc></sitemap>
    ...
    expect(logger.info).toHaveBeenCalledWith("IndexNow: submitting 4 URLs for crewroute.app");
  });
});
