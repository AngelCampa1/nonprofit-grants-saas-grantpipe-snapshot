...
describe("buildIndexNowPayload", () => {
  ...
  it("sets keyLocation to https://{host}/{INDEXNOW_KEY_FILENAME}", () => {
    ...
    expect(payload.keyLocation).toBe(`https://example.com/${INDEXNOW_KEY_FILENAME}`);
  });
});
...
describe("submitToIndexNow", () => {
  it("calls fetch with correct URL, method POST, correct headers, serialized body", async () => {
    ...
    const payload = buildIndexNowPayload("crewroute.app", ["https://crewroute.app/"]);
  ...
  it("returns success=true when fetch returns status 200", async () => {
    ...
    const payload = buildIndexNowPayload("crewroute.app", ["https://crewroute.app/"]);
    const result = await submitToIndexNow(payload, mockFetch as unknown as typeof fetch);
