...
describe("isJsonLdSchema", () => {
  it("returns true for valid JSON-LD schema", () => {
    expect(isJsonLdSchema({ "@context": "https://schema.org", "@type": "Article" })).toBe(true);
  });
  ...
  it("returns false for non-schema.org @context", () => {
    expect(isJsonLdSchema({ "@context": "https://example.com", "@type": "Article" })).toBe(false);
  });
});
...
describe("validateSchema", () => {
  ...
  describe("BreadcrumbList", () => {
    ...
    it("returns error when itemListElement is missing", () => {
      const result = validateSchema({ ...base });
      expect(result.valid).toBe(false);
      expect(result.errors).toContain("BreadcrumbList requires itemListElement");
    });
