...
import { buildRssFeedOptions, contentItemToRssItem, type RssContentItem } from "./rss-utils";
...
describe("buildRssFeedOptions", () => {
  ...
  it("sets description to config.metaDescription when present", () => {
    const result = buildRssFeedOptions(baseConfig, []);
    expect(result.description).toBe("The simplest HVAC dispatch software for owner-operators.");
  });
