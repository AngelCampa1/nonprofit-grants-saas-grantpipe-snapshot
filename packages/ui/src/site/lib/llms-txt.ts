    ...
    .map((section) => ({
      ...
      items: section.entries
        ...
        .map((entry) => ({
          ...
          description: section.description?.(entry) ?? getDefaultLlmsDescription(entry),
        })),
    }))
...
export function buildLlmsTxt(opts: LlmsTxtOptions): string {
  ...
  appendBulletSection(lines, "Product Facts", opts.facts);
  appendBulletSection(lines, "Best For", opts.bestFor);
  appendBulletSection(lines, "Not For", opts.notFor);

  const nonEmptySections = opts.sections.filter((section) => section.items.length > 0);
...
function appendBulletSection(lines: string[], heading: string, items: string[] | undefined): void {
...
function resolveLlmsPath<T>(path: string | ((entry: T) => string), entry: T): string {
  return typeof path === "function" ? path(entry) : path;
}

function normalizeLlmsPath(path: string): string {
  const pathWithLeadingSlash = path.startsWith("/") ? path : `/${path}`;
  const withoutMarkdownSuffix = pathWithLeadingSlash.replace(/\.md$/i, "");
  if (withoutMarkdownSuffix.endsWith("/") || /\.(txt|xml|json)$/i.test(withoutMarkdownSuffix)) {
...
function getDefaultLlmsTitle<T>(entry: T): string {
  ...
  throw new Error("Llms manifest entries without data.title require an explicit title resolver.");
}
