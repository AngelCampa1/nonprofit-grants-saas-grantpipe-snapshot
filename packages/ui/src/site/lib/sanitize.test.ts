...
import { sanitizeJsonLd, sanitizeHref, sanitizeExcerpt, sanitizeHtml } from "./sanitize";
...
describe("sanitizeExcerpt (from sanitize.ts)", () => {
  ...
  it("preserves multiple <mark> tags", () => {
    const result = sanitizeExcerpt("<mark>first</mark> and <mark>second</mark>");
  ...
  it("removes javascript: protocol inside <mark> content", () => {
    const result = sanitizeExcerpt("<mark>visit javascript:alert(1) here</mark>");
    expect(result).not.toContain("javascript:");
...
describe("sanitizeHtml", () => {
  ...
  it("strips object and embed tags", () => {
    const html = '<object data="evil.swf">content</object><embed src="evil.swf" />';
    expect(sanitizeHtml(html)).toBe("");
  });
