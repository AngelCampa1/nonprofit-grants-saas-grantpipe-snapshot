...
import { isQualifiedSurveyResponse, matchesSurveyQualification } from "./survey-qualification";

describe("survey qualification helpers", () => {
  it("returns true when no qualification config is provided", () => {
    expect(matchesSurveyQualification({ role: "Anything" }, undefined)).toBe(true);
  });
  ...
  it("returns false when qualification exists but rules are empty", () => {
    expect(matchesSurveyQualification({ role: "Owner" }, { rules: [] })).toBe(false);
  });
