...
describe("PricebookBuilder", () => {
  describe("rendering", () => {
    ...
    it("renders download button", () => {
      render(<PricebookBuilder {...defaultProps} />);
      expect(screen.getByRole("button", { name: /get your pricebook pdf/i })).toBeDefined();
    });
    ...
    it("selected trade button uses brand-primary token", () => {
      render(<PricebookBuilder {...defaultProps} />);
      // HVAC is selected by default (first trade)
      const hvacBtn = screen.getByRole("button", { name: /hvac/i });
      expect(hvacBtn.className).toContain("brand-primary");
    });

    it("category label uses brand-primary token", () => {
      render(<PricebookBuilder {...defaultProps} />);
      // Category labels are rendered as <td> with the brand-primary class
      const electricalCell = screen.getByText("Electrical");
      expect(electricalCell.className).toContain("brand-primary");
    });
  });
  ...
  describe("inputs", () => {
    ...
    it("calls trackEvent when labor rate changes", () => {
      ...
      expect(trackEvent).toHaveBeenCalledWith(
        "pricebook_builder_inputs_changed",
        expect.objectContaining({ labor_rate_bucket: "150_199" }),
      );
    });
    ...
    it("calls trackEvent when parts markup changes", () => {
      ...
      expect(trackEvent).toHaveBeenCalledWith(
        "pricebook_builder_inputs_changed",
        expect.objectContaining({ parts_markup_bucket: "2_2_99x" }),
      );
    });
    ...
    it("buckets low after-hours multiplier analytics inputs", () => {
      render(<PricebookBuilder {...defaultProps} />);
      const ahInput = screen.getByLabelText(/after-hours multiplier/i);
      fireEvent.change(ahInput, { target: { value: "1.25" } });

      expect(trackEvent).toHaveBeenCalledWith(
        "pricebook_builder_inputs_changed",
        expect.objectContaining({ after_hours_multiplier_bucket: "under_1_5x" }),
      );
    });
    ...
    it("calls trackEvent when after-hours multiplier changes", () => {
      ...
      expect(trackEvent).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          trade: "hvac",
          labor_rate_bucket: "100_149",
          parts_markup_bucket: "3_3_99x",
          after_hours_multiplier_bucket: "2x_plus",
        }),
      );
    });

    it("buckets low labor rate and low markup analytics inputs", () => {
      render(<PricebookBuilder {...defaultProps} />);
      const laborInput = screen.getByLabelText(/billable rate/i);
      const markupInput = screen.getByLabelText(/parts markup/i);
      fireEvent.change(laborInput, { target: { value: "95" } });
      fireEvent.change(markupInput, { target: { value: "1.75" } });

      expect(trackEvent).toHaveBeenCalledWith(
        "pricebook_builder_inputs_changed",
        expect.objectContaining({ labor_rate_bucket: "under_100" }),
      );
      expect(trackEvent).toHaveBeenCalledWith(
        "pricebook_builder_inputs_changed",
        expect.objectContaining({ parts_markup_bucket: "under_2x" }),
      );
    });

    it("buckets high labor rate and high markup analytics inputs", () => {
      render(<PricebookBuilder {...defaultProps} />);
      const laborInput = screen.getByLabelText(/billable rate/i);
      const markupInput = screen.getByLabelText(/parts markup/i);
      fireEvent.change(laborInput, { target: { value: "225" } });
      fireEvent.change(markupInput, { target: { value: "4.5" } });

      expect(trackEvent).toHaveBeenCalledWith(
        "pricebook_builder_inputs_changed",
        expect.objectContaining({ labor_rate_bucket: "200_plus" }),
      );
      expect(trackEvent).toHaveBeenCalledWith(
        "pricebook_builder_inputs_changed",
        expect.objectContaining({ parts_markup_bucket: "4x_plus" }),
      );
    });
  });
  ...
  describe("email gate - submission", () => {
    it("calls fetch with correct payload on valid email submit", async () => {
      ...
      fireEvent.click(screen.getByRole("button", { name: /get your pricebook pdf/i }));
    ...
    it("shows CTA after success", async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByRole("link", { name: /start your free trial/i })).toBeDefined();
      });
    });

    it("reports and shows generic error on non-ok fetch response", async () => {
      ...
      expect(trackEvent).toHaveBeenCalledWith("pricebook_pdf_request_failed", {
        trade: "hvac",
    ...
    it("uses apiUrl prop to construct fetch URL", async () => {
      ...
      render(<PricebookBuilder {...defaultProps} apiUrl="https://custom-api.example" />);
      const emailInput = screen.getByLabelText(/your email.*pdf/i);
      fireEvent.change(emailInput, { target: { value: "tech@hvac.com" } });
      fireEvent.click(screen.getByRole("button", { name: /get your pricebook pdf/i }));
  ...
  describe("trades parameterization", () => {
    it("renders custom trade buttons from trades prop", () => {
      ...
      expect(screen.getByRole("button", { name: "Heating/Cooling" })).toBeDefined();
      expect(screen.getByRole("button", { name: "Plumbing/Gas" })).toBeDefined();
    });
    ...
    it("renders custom download button text from copy prop", () => {
      render(<PricebookBuilder {...defaultProps} copy={{ downloadCtaText: "Download Now" }} />);
      expect(screen.getByRole("button", { name: "Download Now" })).toBeDefined();
    });

    it("renders custom download label from copy prop", () => {
      render(
        <PricebookBuilder {...defaultProps} copy={{ downloadLabel: "Enter your work email" }} />,
      );
    ...
    it("renders custom repairs priced label from copy prop", () => {
      render(
        <PricebookBuilder {...defaultProps} copy={{ repairsPricedLabel: "{count} tasks ready" }} />,
      );
