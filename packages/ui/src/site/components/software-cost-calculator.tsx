...
import { generateComparison, type CompetitorPricing } from "../lib/competitor-cost-calculator";
...
// Competitor-cost-calculator currently emits whole-dollar numbers; convert to
// cents before delegating to the shared canonical formatter.
function formatCurrency(amount: number): string {
  return formatCurrencyCents(Math.round(amount * 100));
}

export function bucketTeamSize(teamSize: number): string {
  if (teamSize <= 3) return "1-3";
  if (teamSize <= 7) return "4-7";
  if (teamSize <= 15) return "8-15";
  return "unknown";
}

export function bucketMonthlySavings(amount: number): string {
...
export function SoftwareCostCalculator({
  ...
  function handleSliderChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setTeamSize(val);
    trackEvent("cost_calculator_team_size_changed", {
      team_size_bucket: bucketTeamSize(val),
    });
  }
  ...
  const competitorResults = results.filter((r) => r.competitor.slug !== productSlug);
  ...
  const mostExpensive = competitorResults.find((r) => r.savingsVsProduct === maxSavings);
  const showSavings = maxSavings > 0 && mostExpensive !== undefined;
  const analyticsProperties = useMemo(
    () => ({
      calculator_id: "software_cost",
      product_slug: productSlug,
      team_size_bucket: bucketTeamSize(teamSize),
      competitor_count: competitors.length,
      savings_bucket: bucketMonthlySavings(maxSavings),
    }),
    [competitors.length, maxSavings, productSlug, teamSize],
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <div
          className="flex justify-between text-brand-muted"
                  ...
                  >
                    ...
                    >
                      <div className="flex flex-col gap-0.5">
                        <span
                          className={clsx(
                            "font-medium",
                            isProduct ? "text-primary-500 font-semibold" : "text-brand-text",
                          )}
                        >
                          {result.competitor.name}
                          {result.isCheapest ? (
                            <span
                              className="ml-2 inline-block text-accent-950 bg-accent-500 font-medium rounded px-1.5 py-0.5"
                    ...
                    <td
                      className={clsx(
                        ...
                        isProduct ? "font-semibold text-primary-500" : "text-brand-text",
                      )}
                      style={{
                        fontSize: "var(--text-body)",
                    ...
                    >
                      ...
                      {result.competitor.setupFee > 0 ? (
                        <span
                          ...
                          style={{ fontSize: "var(--text-caption)" }}
                        >
                          incl. ${result.competitor.setupFee.toLocaleString()} setup
                        </span>
                      ) : null}
                    </td>
                  </tr>
      ...
      {showSavings ? (
        ...
        >
          ...
          >
            {savingsSubTemplate.replace("{annualSavings}", formatCurrency(maxSavings * 12))}
          </p>
        </div>
      ) : null}
        ...
        <a
          ...
          onClick={() => {
            trackEvent("calculator_cta_clicked", {
              ...analyticsProperties,
              destination_path: destinationPathFromHref(trialUrl),
            });
          }}
