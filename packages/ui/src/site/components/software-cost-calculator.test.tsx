...
import {
  ...
  bucketMonthlySavings,
  bucketTeamSize,
} from "./software-cost-calculator";
...
describe("SoftwareCostCalculator", () => {
  describe("analytics buckets", () => {
    it("buckets team size ranges", () => {
      expect(bucketTeamSize(3)).toBe("1-3");
      expect(bucketTeamSize(7)).toBe("4-7");
      expect(bucketTeamSize(15)).toBe("8-15");
      expect(bucketTeamSize(16)).toBe("unknown");
    });

    it("buckets monthly savings ranges", () => {
      expect(bucketMonthlySavings(-1)).toBe("negative");
      expect(bucketMonthlySavings(99)).toBe("under_100");
      expect(bucketMonthlySavings(499)).toBe("100-500");
      expect(bucketMonthlySavings(999)).toBe("500-1k");
      expect(bucketMonthlySavings(4_999)).toBe("1k-5k");
      expect(bucketMonthlySavings(5_000)).toBe("5k_plus");
    });
  });
  ...
  describe("rendering", () => {
    ...
    it("does not show subtitle when productSubtitle is omitted", () => {
      const propsWithoutSubtitle = omitProps(defaultProps, "productSubtitle");
    ...
    it("renders custom CTA text", () => {
      render(<SoftwareCostCalculator {...defaultProps} ctaText="Get Started Now →" />);
  ...
  describe("slider interaction", () => {
    ...
    it("updates the savings callout when slider changes", () => {
      ...
      expect(screen.getByText(/at 10 techs.*crewroute saves you/i)).toBeDefined();
    ...
    it("calls trackEvent when slider changes", () => {
      ...
      expect(trackEvent).toHaveBeenCalledWith("cost_calculator_team_size_changed", {
        team_size_bucket: "4-7",
      });
    });

    it("calls trackEvent with a bucketed team size", () => {
      render(<SoftwareCostCalculator {...defaultProps} />);
      const slider = screen.getByRole("slider");
      fireEvent.change(slider, { target: { value: "10" } });
      expect(trackEvent).toHaveBeenCalledWith("cost_calculator_team_size_changed", {
        team_size_bucket: "8-15",
      });
    });
    ...
    it("tracks result views with bucketed calculator properties", () => {
      ...
      expect(trackEvent).toHaveBeenCalledWith("calculator_result_viewed", {
        calculator_id: "software_cost",
        product_slug: "crewroute",
        team_size_bucket: "1-3",
        competitor_count: 5,
        savings_bucket: "500-1k",
      });
    });

    it("does not track another result view when slider recalculates savings", () => {
      render(<SoftwareCostCalculator {...defaultProps} />);
      const slider = screen.getByRole("slider");

      fireEvent.change(slider, { target: { value: "7" } });

      const resultViews = vi
        .mocked(trackEvent)
        .mock.calls.filter(([eventName]) => eventName === "calculator_result_viewed");
  ...
  describe("CTA", () => {
    ...
    it("CTA text contains 'No Setup Fees'", () => {
      render(<SoftwareCostCalculator {...defaultProps} />);
      expect(screen.getByText(/start your free trial.*no setup fees/i)).toBeDefined();
    });

    it("tracks CTA clicks without URL query strings", () => {
      render(
        <SoftwareCostCalculator
          {...defaultProps}
          trialUrl="https://app.example.com/signup?token=secret"
        />,
      );
      ...
      expect(trackEvent).toHaveBeenCalledWith("calculator_cta_clicked", {
        calculator_id: "software_cost",
        product_slug: "crewroute",
        team_size_bucket: "1-3",
        competitor_count: 5,
      ...
      expect(JSON.stringify(vi.mocked(trackEvent).mock.calls)).not.toContain("token=secret");
    });
  });
  ...
  describe("formatting", () => {
    it("monthly cost values show $ prefix", () => {
      render(<SoftwareCostCalculator {...defaultProps} />);
      const cells = screen.getAllByRole("cell").filter((cell) => cell.textContent?.startsWith("$"));
  ...
  describe("parameterization", () => {
    ...
    it("renders without error when only the product slug is in competitors", () => {
      ...
      render(<SoftwareCostCalculator {...defaultProps} competitors={soloCompetitor} />);
      expect(screen.getByText("CrewRoute")).toBeDefined();
