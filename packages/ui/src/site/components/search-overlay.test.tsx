...
describe("sanitizeExcerpt", () => {
  ...
  it("preserves <mark> tags", () => {
    expect(sanitizeExcerpt("foo <mark>bar</mark> baz")).toBe("foo <mark>bar</mark> baz");
  });
  ...
  it("escapes tags with attributes but preserves mark", () => {
    ...
    const result = sanitizeExcerpt('<span class="x">hello</span> <mark>world</mark>');
  ...
  it("strips attributes from mark tags — onerror on mark is XSS", () => {
    expect(sanitizeExcerpt('<mark onerror="alert(1)">text</mark>')).toBe("<mark>text</mark>");
  });

  it("strips onclick attribute from mark tag", () => {
    expect(sanitizeExcerpt('<mark onclick="evil()">found</mark>')).toBe("<mark>found</mark>");
  });
  ...
  it("strips nested HTML tags inside mark content", () => {
    ...
    expect(sanitizeExcerpt("<mark><em>nested</em></mark>")).toBe("<mark>nested</mark>");
  });
});
...
describe("SearchOverlay", () => {
  ...
  it("opens overlay on button click", () => {
    render(<SearchOverlay siteName="TestSite" labels={{ emptyState: "Start typing to search" }} />);
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("site_search_opened", {
      trigger: "button",
    });
  });

  it("uses custom placeholder", () => {
    render(<SearchOverlay siteName="TestSite" placeholder="Find articles..." />);
    fireEvent.click(screen.getByRole("button", { name: "Search TestSite" }));
  ...
  it("closes on Escape", () => {
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("site_search_closed", {
      trigger: "escape",
      had_query: false,
      had_results: false,
  ...
  it("tracks search close after a query without sending raw search text", async () => {
    const mockResult = {
      url: "/grant-guide",
      meta: { title: "Grant Guide" },
    ...
    expect(JSON.stringify(mockTrackEvent.mock.calls)).not.toContain("grant compliance");
    expect(JSON.stringify(mockTrackEvent.mock.calls)).not.toContain("Grant Guide");
  });
  ...
  it("shows no-results message when query has no matches", async () => {
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("site_search_performed", {
  ...
  it("renders search results when pagefind returns matches", async () => {
    ...
    const { container } = render(<SearchOverlay siteName="TestSite" _loadPagefind={mockLoader} />);
    fireEvent.click(screen.getByRole("button", { name: "Search TestSite" }));
  ...
  it("buckets mid-size search queries and result counts", async () => {
    const results = Array.from({ length: 4 }, (_value, index) => ({
      url: `/result-${index}`,
    ...
    const mockPagefind = {
      search: vi.fn().mockResolvedValue({
        results: results.map((result) => ({ data: () => Promise.resolve(result) })),
      }),
    };
    const mockLoader = vi.fn().mockResolvedValue(mockPagefind);
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("site_search_performed", {
      query_length_bucket: "11-30",
      result_count_bucket: "4-8",
      had_partial_failure: false,
      max_results: 8,
    });
  });

  it("buckets long search queries and large result counts", async () => {
    const results = Array.from({ length: 9 }, (_value, index) => ({
      url: `/large-result-${index}`,
      meta: { title: `Large Result ${index}` },
      excerpt: "A result excerpt",
    }));
    const mockPagefind = {
      search: vi.fn().mockResolvedValue({
    ...
    render(<SearchOverlay siteName="TestSite" maxResults={12} _loadPagefind={mockLoader} />);
    fireEvent.click(screen.getByRole("button", { name: "Search TestSite" }));
    await act(async () => {});

    fireEvent.change(screen.getByLabelText("Search query"), {
      target: { value: "grant compliance reporting deadline evidence" },
    });
    await act(async () => {
      vi.advanceTimersByTime(300);
    });
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("site_search_performed", {
      query_length_bucket: "30+",
      result_count_bucket: "8+",
      had_partial_failure: false,
      max_results: 12,
    });
  });

  it("clears displayed results when the query is emptied", async () => {
    const mockResult = {
      url: "/some-page",
      meta: { title: "Some Page Title" },
      excerpt: "",
    ...
    expect(screen.queryByText("Some Page Title")).toBeNull();
  ...
  it("resets loading to false when pf.search() throws", async () => {
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("site_search_failed", {
      query_length_bucket: "1-10",
    ...
    expect(captureException).toHaveBeenCalledWith(error, {
      ...
      extra: { query_length_bucket: "1-10", max_results: 8 },
    });
  });
  ...
  it("resets loading to false when a result data() call rejects", async () => {
    ...
    const mockPagefind = {
      search: vi.fn().mockResolvedValue({
        results: [{ data: () => Promise.reject(new Error("data load failure")) }],
      }),
    };
    const mockLoader = vi.fn().mockResolvedValue(mockPagefind);
  ...
  it("shows an error and reports when Pagefind cannot be loaded", async () => {
    const mockLoader = vi.fn().mockResolvedValue(null);

    render(
      <SearchOverlay
        siteName="TestSite"
        labels={{ errorMessage: "Search is unavailable." }}
        _loadPagefind={mockLoader}
      />,
    );
    fireEvent.click(screen.getByRole("button", { name: "Search TestSite" }));
    ...
    expect(screen.getByText("Search is unavailable.")).toBeDefined();
    expect(screen.queryByText("Searching...")).toBeNull();
    expect(captureException).toHaveBeenCalledWith(expect.any(Error), {
  ...
  it("Enter activates the focused result", async () => {
    ...
    expect(screen.queryByRole("dialog")).toBeNull();
    expect(mockTrackEvent).toHaveBeenCalledWith("site_search_result_clicked", {
      query_length_bucket: "1-10",
      result_position: 1,
      result_path: "/enter-page",
      activation_method: "keyboard",
    });
  });
  ...
  it("active index resets when query changes", async () => {
    const mockResults1 = [{ url: "/page-1", meta: { title: "Page One" }, excerpt: "First" }];
    const mockResults2 = [{ url: "/page-a", meta: { title: "Page Alpha" }, excerpt: "Alpha" }];
    let callCount = 0;
    const mockPagefind = {
  ...
  it("limits results to maxResults prop when provided", async () => {
    ...
    const { container } = render(
      <SearchOverlay siteName="TestSite" maxResults={5} _loadPagefind={mockLoader} />,
    );
    fireEvent.click(screen.getByRole("button", { name: "Search TestSite" }));
    await act(async () => {});
  ...
  it("clamps maxResults=0 to 1 so at least one result is shown", async () => {
    ...
    const { container } = render(
      <SearchOverlay siteName="TestSite" maxResults={0} _loadPagefind={mockLoader} />,
    );
    fireEvent.click(screen.getByRole("button", { name: "Search TestSite" }));
    await act(async () => {});
  ...
  it("useFocusTrap receives a ref whose current points to the search panel div", () => {
    ...
    const panelEl = container.querySelector(".relative.w-full.max-w-lg") as HTMLElement;
  ...
  it("calls pagefind destroy() on unmount if defined", async () => {
    ...
    const { unmount } = render(<SearchOverlay siteName="TestSite" _loadPagefind={mockLoader} />);
  ...
  it("closes overlay when a result item is clicked", async () => {
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("site_search_result_clicked", {
      query_length_bucket: "1-10",
      result_position: 1,
      result_path: "/some-page",
      activation_method: "click",
    });
  });

  it("tracks invalid result URLs without throwing", async () => {
    const mockResult = {
      url: "http://[invalid-url",
      meta: { title: "Broken URL" },
      excerpt: "",
    };
    const mockPagefind = {
      search: vi.fn().mockResolvedValue({
  ...
  it("retries search after pagefind loads when query was typed before pagefind was ready", async () => {
    ...
    let resolveLoader: (pf: { search: ReturnType<typeof vi.fn>; destroy?: () => void }) => void;
