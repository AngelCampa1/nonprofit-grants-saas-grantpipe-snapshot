...
vi.mock("./email-capture", () => ({
  EmailCapture: (props: Record<string, unknown>) => (
    <div
      ...
      data-qualified-dismiss-text={props.qualifiedDismissText as string | undefined}
      data-unqualified-dismiss-text={props.unqualifiedDismissText as string | undefined}
...
describe("FakeDoorPricing", () => {
  ...
  it("highlighted tier has border-2 accent styling (not border-t-4)", () => {
    ...
    const proCard = Array.from(cards).find((el) => el.textContent?.includes("Pro"));
  ...
  it("after click, confirmation message appears when confirmationMessage is provided", async () => {
    ...
    render(<FakeDoorPricing {...defaultProps} confirmationMessage="Thanks for your interest!" />);
    fireEvent.click(screen.getByRole("button", { name: "Choose Pro" }));
  ...
  it("uses custom confirmationMessage when provided", async () => {
    ...
    render(<FakeDoorPricing {...defaultProps} confirmationMessage="Your trial is ready!" />);
    fireEvent.click(screen.getByRole("button", { name: "Choose Pro" }));
  ...
  it("reports non-ok pricing click responses without tracking success", async () => {
    ...
    expect(trackEvent).not.toHaveBeenCalledWith("pricing_tier_clicked", expect.anything());
  });
  ...
  it("passes qualification rules through to the modal email capture", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("email-capture").getAttribute("data-qualification")).toBe(
        JSON.stringify({
          logic: "any",
          rules: [{ questionId: "segment", answers: ["Women 40+"] }],
        }),
  ...
  it("uses custom buttonPrefix when provided", () => {
    ...
    expect(screen.getByRole("button", { name: "Get Access to Starter" })).toBeDefined();
    expect(screen.getByRole("button", { name: "Get Access to Pro" })).toBeDefined();
    expect(screen.getByRole("button", { name: "Get Access to Enterprise" })).toBeDefined();
  });
  ...
  it("uses tier.ctaText as button label when set, ignoring buttonPrefix + name", () => {
    ...
    expect(screen.getByRole("button", { name: "Choose Enterprise" })).toBeDefined();
  });

  it("shows only tier name when buttonPrefix is not provided", () => {
    const propsWithoutPrefix = omitProps(defaultProps, "buttonPrefix");
  ...
  it("selected tier card gets selected highlight class", async () => {
    ...
    await waitFor(() => {
      ...
      expect(starterCard?.className).toContain("border-accent-400");
    });
  });
  ...
  it("does not render heading when heading prop is omitted", () => {
    const propsWithoutHeading = omitProps(defaultProps, "heading");
  ...
  it("defaults popularBadgeText to 'Most Popular' when popularTier is set but popularBadgeText is not passed", () => {
    const propsWithoutBadgeText = omitProps(defaultProps, "popularBadgeText");
  ...
  it("does not render description element when tier has no description", () => {
    ...
    const starterCard = screen.getByText("Starter").closest("div") as HTMLElement;
  ...
  it("shows tier-specific message from selectedMessages after selection", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("The Pro plan is perfect for growing teams!")).toBeDefined();
    });
  });
  ...
  it("uses custom selectedBadgeText when provided", async () => {
    ...
    await waitFor(() => {
      ...
      const buttons = screen.getAllByRole("button");
      const selectedButton = buttons.find((b) => b.textContent?.includes("Picked"));
  ...
  it("hides recommendedBadgeText badge on highlighted tier after it is selected", async () => {
    ...
    render(<FakeDoorPricing {...defaultProps} recommendedBadgeText="Top Pick" />);
    fireEvent.click(screen.getByRole("button", { name: "Choose Pro" }));
  ...
  it("renders socialProofText below the tier grid when provided", () => {
    render(<FakeDoorPricing {...defaultProps} socialProofText="47 founders already joined" />);
  ...
  it("card corner badge shows selectedBadgeText prop value, not hardcoded SELECTED", async () => {
    ...
    await waitFor(() => {
      ...
      const badges = screen.getAllByText("Picked").filter((el) => el.tagName === "SPAN");
  ...
  it("non-highlighted unselected tier button has btn-primary class", () => {
    ...
    const starterButton = Array.from(buttons).find((b) => b.textContent?.includes("Starter"));
  ...
  it("highlighted unselected tier button has btn-primary and btn-primary--pulse classes", () => {
    const { container } = render(<FakeDoorPricing {...defaultProps} />);
    const buttons = container.querySelectorAll("button");
    const proButton = Array.from(buttons).find((b) => b.textContent?.includes("Pro"));
  ...
  it("selected tier button has btn-secondary class (not btn-primary)", async () => {
    ...
    await waitFor(() => {
      ...
      const starterBtns = Array.from(buttons).filter((b) => b.className.includes("bg-accent-100"));
  ...
  it("badge priority: a tier cannot show both 'Selected' and 'Most Popular' simultaneously", async () => {
    ...
    await waitFor(() => {
      ...
      expect(selectedBadges.length === 0 || popularBadges.length === 0).toBe(true);
    });
  });
});
...
describe("FakeDoorPricing — clear selection", () => {
  ...
  it("Clear button resets all selections", async () => {
    ...
    expect(screen.getByRole("button", { name: "Choose Starter" })).toBeDefined();
    expect(screen.getByRole("button", { name: "Choose Pro" })).toBeDefined();
  });
...
describe("FakeDoorPricing — emailCapture prop (pay-intent flow)", () => {
  ...
  it("does NOT render EmailCapture before any tier is selected", () => {
    render(<FakeDoorPricing {...defaultProps} emailCapture={emailCaptureProps} />);
  ...
  it("opens a loading modal while the email capture config is still resolving", async () => {
    ...
    render(<FakeDoorPricing {...defaultProps} emailCaptureConfigUrl="/signup-flow.json" />);
    ...
    await waitFor(() => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        "https://api.test/api/pricing-click",
  ...
  it("reuses the loaded email capture config when the modal is opened again", async () => {
    const fetchMock = vi
      .fn()
    ...
    await waitFor(() => {
      expect(screen.getByTestId("email-capture")).toBeDefined();
      expect(fetchMock.mock.calls.filter(([url]) => url === "/signup-flow.json")).toHaveLength(1);
    });
  });
  ...
  it("shows a retry state when the config fetch fails but still records the pricing click", async () => {
    ...
    const fetchMock = vi.fn((url: string) => {
      ...
      if (url === "https://api.test/api/pricing-click") {
        return Promise.resolve({ ok: true });
      }
      return Promise.resolve({ ok: true });
    });
  ...
  it("shows the same retry state when the config endpoint responds with a non-ok status", async () => {
    const fetchMock = vi.fn((url: string) => {
      if (url === "/signup-flow.json") {
  ...
  it("retries loading the email capture config from the modal error state", async () => {
    const fetchMock = vi
      .fn((url: string) => {
        if (url === "/signup-flow.json") {
          return Promise.resolve({ ok: false, status: 500 });
  ...
  it("does not render confirmation paragraph when confirmationMessage is omitted and no selectedMessages match", async () => {
    ...
    const propsWithoutConfirmation = omitProps(defaultProps, "confirmationMessage");
  ...
  it("modal has role=dialog with aria-modal and aria-label when emailCapture tier is selected", async () => {
    ...
    await waitFor(() => {
      ...
      expect(dialog.getAttribute("aria-label")).toBe("Choose your plan and continue");
    });
  });
  ...
  it("forwards extended props (privacyNote, survey qualification copy) to EmailCapture via spread", async () => {
    ...
    await waitFor(() => {
      const el = screen.getByTestId("email-capture");
      expect(el.getAttribute("data-privacy-note")).toBe("We respect your privacy.");
      expect(el.getAttribute("data-error-invalid-email")).toBe("Bad email format");
  ...
  it("defaults EmailCapture buttonText to 'Join launch access' when not explicitly provided", async () => {
    ...
    render(<FakeDoorPricing {...defaultProps} emailCapture={minimalEmailCapture} />);
    fireEvent.click(screen.getByRole("button", { name: "Choose Starter" }));
    await waitFor(() => {
      const el = screen.getByTestId("email-capture");
      expect(el.getAttribute("data-button-text")).toBe("Join launch access");
    });
  });

  it("defaults EmailCapture subtitle to a launch-access disclosure with productName when not explicitly provided", async () => {
    ...
    render(<FakeDoorPricing {...defaultProps} emailCapture={emailCaptureWithProduct} />);
    fireEvent.click(screen.getByRole("button", { name: "Choose Starter" }));
    await waitFor(() => {
      const el = screen.getByTestId("email-capture");
      expect(el.getAttribute("data-subtitle")).toBe(
        "You picked a plan. Enter your email to join launch access for CrewRoute.",
      );
    });
  });

  it("defaults EmailCapture subtitle to a generic launch-access disclosure when no productName and no explicit subtitle", async () => {
    const minimalEmailCapture = {
      apiUrl: "https://api.test",
      sourcePage: "/pricing",
      surveyQuestions: [] as SurveyQuestion[],
  ...
  it("uses the modal aria label for the nested EmailCapture by default", async () => {
    ...
    await waitFor(() => {
      const el = screen.getByTestId("email-capture");
      expect(el.getAttribute("data-aria-label")).toBe("Choose your plan and continue");
    });
  });
  ...
  it("uses explicit buttonText override when provided in emailCapture", async () => {
    ...
    render(<FakeDoorPricing {...defaultProps} emailCapture={customEmailCapture} />);
    fireEvent.click(screen.getByRole("button", { name: "Choose Starter" }));
    await waitFor(() => {
      const el = screen.getByTestId("email-capture");
  ...
  it("forwards surveyPreview to EmailCapture", async () => {
    ...
    render(<FakeDoorPricing {...defaultProps} emailCapture={captureWithSurvey} />);
    fireEvent.click(screen.getByRole("button", { name: "Choose Starter" }));
    await waitFor(() => {
      const el = screen.getByTestId("email-capture");
...
describe("FakeDoorPricing — billing toggle", () => {
  ...
  it("no toggle rendered when showBillingToggle is false even with monthlyPriceCents", () => {
    render(
      <FakeDoorPricing {...defaultProps} tiers={tiersWithMonthly} showBillingToggle={false} />,
    );
    expect(screen.queryByRole("radiogroup")).toBeNull();
  });
  ...
  it("trial banner renders when trialBannerText is provided", () => {
    ...
    expect(screen.getByText("Pick a plan to see pricing details and next steps.")).toBeDefined();
  });

  it("trial banner is absent when trialBannerText is not provided", () => {
    render(<FakeDoorPricing {...defaultProps} tiers={tiersWithMonthly} />);
    expect(screen.queryByText("Pick a plan to see pricing details and next steps.")).toBeNull();
  });

  it("preserves allowed free-trial banner copy before rendering", () => {
    render(
      <FakeDoorPricing
        {...defaultProps}
        tiers={tiersWithMonthly}
    ...
    expect(screen.getByText("1-month free trial included")).toBeDefined();
  });
  ...
  it("struck-through monthly price appears in annual mode for tiers with monthlyPriceCents", async () => {
    ...
    await waitFor(() => {
      ...
      const struckThrough = allStarterPrice.find((el) => el.className.includes("line-through"));
  ...
  it("Annual radio becomes aria-checked=true after clicking it", async () => {
    ...
    fireEvent.click(screen.getByRole("radio", { name: "Annual" }));
    await waitFor(() => {
      expect(screen.getByRole("radio", { name: "Annual" }).getAttribute("aria-checked")).toBe(
        "true",
      );
  ...
  it("billing period radios keep a mobile-safe minimum tap height", () => {
    ...
    expect(screen.getByRole("radio", { name: "Monthly" }).className).toContain("min-h-11");
    expect(screen.getByRole("radio", { name: "Annual" }).className).toContain("min-h-11");
  });
  ...
  it("shows per-month equivalent label below annual price in annual mode", async () => {
    // Tier with monthlyPriceCents: 4900 yields $490/yr and a rounded-up ~$41/mo equivalent.
    render(<FakeDoorPricing {...defaultProps} tiers={tiersWithMonthly} />);
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("~$41/mo")).toBeDefined();
    });
  });
  ...
  it("per-month equivalent label is absent in monthly mode", () => {
    ...
    expect(screen.queryByText("~$41/mo")).toBeNull();
  });
});
...
describe("FakeDoorPricing — clearButtonText prop", () => {
  ...
  it("calls trackBillingToggle when monthly toggle is clicked", () => {
    ...
    render(<FakeDoorPricing {...defaultProps} tiers={tiersWithAnnual} showBillingToggle={true} />);
    fireEvent.click(screen.getByRole("radio", { name: "Monthly" }));
  ...
  describe("open-pricing-modal CustomEvent", () => {
    ...
    it("signals that the pricing modal is ready after mount", async () => {
      ...
      await waitFor(() => {
        ...
        expect(document.documentElement.dataset.fakeDoorPricingReady).toBe("true");
      });
    ...
    it("opens modal, pre-selects first tier, and tracks the first tier when event is dispatched with emailCapture prop", async () => {
      ...
      await waitFor(() => {
        expect(screen.queryAllByText("Selected").length).toBeGreaterThanOrEqual(1);
      });
    ...
    it("does nothing when open-pricing-modal is dispatched with empty tiers", async () => {
      render(<FakeDoorPricing {...defaultProps} tiers={[]} emailCapture={emailCaptureProps} />);
    ...
    it("has data-fake-door-pricing attribute on the outermost element", () => {
      ...
      expect(container.querySelector("[data-fake-door-pricing]")).not.toBeNull();
    });
