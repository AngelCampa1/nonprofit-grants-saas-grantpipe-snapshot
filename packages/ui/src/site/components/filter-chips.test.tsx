...
describe("FilterChips", () => {
  ...
  it("clears all filters on Clear click", () => {
    ...
    expect(screen.queryByText("Clear")).toBeNull();
    expect(screen.getByText("Awareness").getAttribute("aria-pressed")).toBe("false");
  });
  ...
  it("renders sort dropdown when sortOptions provided", () => {
    ...
    render(<FilterChips {...defaultProps} sortOptions={sortOptions} defaultSort="newest" />);
  ...
  it("switches between filter values in the same group", () => {
    render(<FilterChips {...defaultProps} />);
    fireEvent.click(screen.getByText("Awareness"));
    expect(screen.getByText("Awareness").getAttribute("aria-pressed")).toBe("true");

    fireEvent.click(screen.getByText("Decision"));
    expect(screen.getByText("Decision").getAttribute("aria-pressed")).toBe("true");
    expect(screen.getByText("Awareness").getAttribute("aria-pressed")).toBe("false");
  ...
  it("calls onFilterChange with active filters when a filter is clicked", () => {
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("resource_filter_changed", {
      filter_id: "buyerStage",
      filter_value: "tofu",
      action: "selected",
      active_filter_count: 1,
      result_count_bucket: "1-3",
      total_count_bucket: "4-10",
    });
  });
  ...
  it("calls onFilterChange with empty object when filter is deselected", () => {
    ...
    expect(mockTrackEvent).toHaveBeenLastCalledWith("resource_filter_changed", {
      filter_id: "buyerStage",
      filter_value: "tofu",
      action: "cleared",
  ...
  it("calls onSortChange when sort select changes", () => {
    ...
    render(<FilterChips {...defaultProps} sortOptions={sortOptions} onSortChange={onSortChange} />);
    const select = screen.getByLabelText("Sort results:") as HTMLSelectElement;
    fireEvent.change(select, { target: { value: "oldest" } });
    expect(onSortChange).toHaveBeenCalledTimes(1);
    expect(onSortChange).toHaveBeenCalledWith("oldest");
    expect(mockTrackEvent).toHaveBeenCalledWith("resource_sort_changed", {
      sort_value: "oldest",
      active_filter_count: 0,
      result_count_bucket: "4-10",
      total_count_bucket: "4-10",
  ...
  it("calls onItemsFiltered with filtered items when filter is active", () => {
    ...
    const lastCall = onItemsFiltered.mock.calls[onItemsFiltered.mock.calls.length - 1];
  ...
  it("calls onFilterChange with empty object when Clear is clicked", () => {
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("resource_filters_cleared", {
      cleared_filter_count: 1,
      result_count_bucket: "4-10",
      total_count_bucket: "4-10",
    });
  });

  it("tracks zero-result filters with the 0 result count bucket", () => {
    ...
    render(<FilterChips filters={metadataFilters} items={items} onItemsFiltered={vi.fn()} />);
  ...
  it("tracks large resource result sets with the 10+ count bucket", () => {
    const manyItems: ContentItem[] = Array.from({ length: 11 }, (_value, index) => ({
      buyerStage: "tofu",
      title: `Article ${index}`,
      description: "Desc",
      href: `/article-${index}`,
    ...
    render(<FilterChips filters={filters} items={manyItems} onItemsFiltered={vi.fn()} />);
    fireEvent.click(screen.getByText("Awareness"));
    ...
    expect(mockTrackEvent).toHaveBeenCalledWith("resource_filter_changed", {
      ...
      action: "selected",
      active_filter_count: 1,
      result_count_bucket: "10+",
      total_count_bucket: "10+",
    });
  });
  ...
  it("does not throw when onFilterChange is not provided", () => {
    ...
    expect(() => {
      render(<FilterChips filters={filters} items={items} onItemsFiltered={vi.fn()} />);
      fireEvent.click(screen.getByText("Awareness"));
    }).not.toThrow();
  });
  ...
  it("calls onItemsFiltered with updated items when items prop changes", () => {
    ...
    const { rerender } = render(
      <FilterChips filters={filters} items={initialItems} onItemsFiltered={onItemsFiltered} />,
    );
    ...
    rerender(<FilterChips filters={filters} items={newItems} onItemsFiltered={onItemsFiltered} />);

    // Must be called again with the new items
    expect(onItemsFiltered.mock.calls.length).toBeGreaterThan(callCountAfterMount);
    const lastCall = onItemsFiltered.mock.calls[onItemsFiltered.mock.calls.length - 1];
  ...
  it("calls onItemsFiltered with filtered items (not all) when items prop changes while filter is active", () => {
    const onItemsFiltered = vi.fn();
    const { rerender } = render(
      <FilterChips filters={filters} items={items} onItemsFiltered={onItemsFiltered} />,
    );
    ...
    expect(
      (lastCall[0] as Array<{ buyerStage: string }>).every((item) => item.buyerStage === "tofu"),
    ).toBe(true);
  });
  ...
  it("each filter group div has role='group'", () => {
    ...
    const { container } = render(<FilterChips {...defaultProps} filters={multiFilters} />);
  ...
  it("filters by metadata key when filter id is not buyerStage or featured", () => {
    ...
    expect(
      (lastCall[0] as ContentItem[]).every((item) => item.metadata?.category === "guide"),
    ).toBe(true);
  });
