...
describe("PostSignupSurvey", () => {
  ...
  it("shows Got it button when qualifyCriteria returns false", async () => {
    ...
    render(<PostSignupSurvey {...defaultProps} qualifyCriteria={() => false} />);
    fireEvent.click(screen.getByText("Dev"));
    fireEvent.click(screen.getByText("Excel"));
  ...
  it("does not render unqualifiedHeading when prop is omitted", async () => {
    ...
    const propsWithoutHeading = omitProps(defaultProps, "unqualifiedHeading");
    render(<PostSignupSurvey {...propsWithoutHeading} qualifyCriteria={() => false} />);
    fireEvent.click(screen.getByText("Dev"));
    fireEvent.click(screen.getByText("Excel"));
  ...
  it("does not render unqualifiedBody when prop is omitted", async () => {
    ...
    const propsWithoutBody = omitProps(defaultProps, "unqualifiedBody");
    render(<PostSignupSurvey {...propsWithoutBody} qualifyCriteria={() => false} />);
    fireEvent.click(screen.getByText("Dev"));
    fireEvent.click(screen.getByText("Excel"));
  ...
  it("renders qualifiedBody when prop is provided", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Book a 15-min walkthrough to get started.")).toBeDefined();
    });
  });
  ...
  it("does not render qualified dismiss button when qualifiedDismissText is omitted", async () => {
    ...
    const propsWithout = omitProps(defaultProps, "qualifiedDismissText");
  ...
  it("does not render unqualified CTA link when unqualifiedCtaText is omitted", async () => {
    ...
    const propsWithout = omitProps(defaultProps, "unqualifiedCtaText");
    render(<PostSignupSurvey {...propsWithout} qualifyCriteria={() => false} />);
    fireEvent.click(screen.getByText("Dev"));
    fireEvent.click(screen.getByText("Excel"));

    await waitFor(() => screen.getByRole("dialog", { name: "Survey complete" }));
    expect(screen.queryByRole("link", { name: "Explore our guides" })).toBeNull();
  });
  ...
  it("both primary CTA link and dismiss button render in unqualified path", async () => {
    ...
    await waitFor(() => {
      // Primary CTA link
      expect(screen.getByRole("link", { name: "Explore our guides" })).toBeDefined();
  ...
  it("primary CTA link in unqualified path has accent styling", async () => {
    ...
    await waitFor(() => screen.getByRole("link", { name: "Explore our guides" }));
    const ctaLink = screen.getByRole("link", { name: "Explore our guides" });
    expect(ctaLink.className).toContain("btn-primary");
  });
  ...
  it("does not render CTA link when both unqualifiedCtaText and unqualifiedCtaTarget are omitted", async () => {
    ...
    const propsWithout = omitProps(defaultProps, "unqualifiedCtaText", "unqualifiedCtaTarget");
    render(<PostSignupSurvey {...propsWithout} qualifyCriteria={() => false} />);
    fireEvent.click(screen.getByText("Dev"));
    fireEvent.click(screen.getByText("Excel"));
  ...
  it("does not render CTA link when unqualifiedCtaText is provided but unqualifiedCtaTarget is omitted", async () => {
    ...
    const propsWithout = omitProps(defaultProps, "unqualifiedCtaTarget");
    render(<PostSignupSurvey {...propsWithout} qualifyCriteria={() => false} />);
    fireEvent.click(screen.getByText("Dev"));
    fireEvent.click(screen.getByText("Excel"));
  ...
  it("shows spinner SVG and 'Submitting...' on last-question button during submission", async () => {
    ...
    await waitFor(() => {
      ...
      const optionButtons = buttons.filter(
        (b) => b.textContent !== "" && b.getAttribute("aria-label") !== "Close survey",
      );
  ...
  it("renders nothing (returns null) when questions array is empty", () => {
    const { container } = render(<PostSignupSurvey {...defaultProps} questions={[]} />);
  ...
  it("progress bar shows 0% on question 1 of 3 (step 0)", () => {
    ...
    const { container } = render(<PostSignupSurvey {...defaultProps} questions={threeQuestions} />);
    // step 0, questions.length 3 → 0/3 * 100 = 0%
    const progressBar = container.querySelector(
      ".h-full.bg-accent-400.transition-\\[transform\\]",
    ) as HTMLElement;
  ...
  it("does not crash when questions array changes from non-empty to empty", () => {
    ...
    expect(() => rerender(<PostSignupSurvey {...defaultProps} questions={[]} />)).not.toThrow();
  });
  ...
  it("retry success: shows completion screen after successful retry", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockRejectedValueOnce(new Error("fail")).mockResolvedValueOnce({ ok: true }),
    );
  ...
  it("uses serializable qualification config to show the qualified path", async () => {
    ...
    render(<PostSignupSurvey {...defaultProps} qualification={fortyPlusQualification} />);
    fireEvent.click(screen.getByText("Dev"));
    fireEvent.click(screen.getByText("Excel"));
  ...
  it("does not fire trackEvent when fetch fails", async () => {
    ...
    expect(trackEvent).not.toHaveBeenCalledWith("survey_completed", expect.anything());
  });
  ...
  it("skips API call and shows completion screen when surveyToken is undefined", async () => {
    ...
    const propsWithoutToken = omitProps(defaultProps, "surveyToken");
