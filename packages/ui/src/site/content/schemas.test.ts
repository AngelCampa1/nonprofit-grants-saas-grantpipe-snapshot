...
describe("baseContentSchema", () => {
  ...
  it("accepts freshness, CTA, intent, keyword, and source metadata", () => {
    const result = baseContentSchema.parse({
      ...
      targetKeyword: "grant management software",
      sourceUrls: ["https://example.com/source"],
    });
    expect(result.lastReviewedAt).toBe("2026-04-21");
    expect(result.verifiedAt).toBe("2026-04-21");
    expect(result.primaryCta).toBe("trial");
    expect(result.contentIntent).toBe("comparison");
    expect(result.topicCluster).toBe("grant-management");
    expect(result.refreshCadenceMonths).toBe(6);
    expect(result.targetKeyword).toBe("grant management software");
    expect(result.sourceUrls).toEqual(["https://example.com/source"]);
  });

  it("accepts an explicit leadMagnetSlug override", () => {
    const result = baseContentSchema.parse({
      ...validBase,
      leadMagnetSlug: "donor-retention-playbook",
    });

    expect(result.leadMagnetSlug).toBe("donor-retention-playbook");
  });

  it("accepts an explicit exitMagnetSlug override", () => {
    const result = baseContentSchema.parse({
      ...validBase,
      exitMagnetSlug: "nonprofit-crm-evaluation-scorecard",
    });

    expect(result.exitMagnetSlug).toBe("nonprofit-crm-evaluation-scorecard");
  });

  it("rejects an invalid exitMagnetSlug value", () => {
    expect(() =>
      baseContentSchema.parse({ ...validBase, exitMagnetSlug: "not-a-real-slug" }),
    ).toThrow();
  });

  it("rejects invalid topicCluster values", () => {
    expect(() =>
      baseContentSchema.parse({
        ...validBase,
        topicCluster: "invalid-cluster",
      }),
    ).toThrow();
  });
  ...
  it("rejects empty relatedPages array", () => {
    expect(() => baseContentSchema.parse({ ...validBase, relatedPages: [] })).toThrow();
  });
  ...
  it("rejects invalid buyerStage", () => {
    expect(() => baseContentSchema.parse({ ...validBase, buyerStage: "invalid" })).toThrow();
  });
  ...
  it("accepts optional seoTitle and seoDescription overrides", () => {
    const result = baseContentSchema.parse({
      ...validBase,
      seoTitle: "Grant Management Software for Nonprofits",
      seoDescription: "GrantPipe keeps donors, grants, and compliance reporting in one workflow.",
    });
    expect(result.seoTitle).toBe("Grant Management Software for Nonprofits");
    expect(result.seoDescription).toBe(
      "GrantPipe keeps donors, grants, and compliance reporting in one workflow.",
    );
  });
  ...
  it("defaults seoTitle and seoDescription to undefined when omitted", () => {
    const result = baseContentSchema.parse(validBase);
  ...
  it("accepts targetPersona as an array of strings", () => {
    ...
    expect(result.targetPersona).toEqual(["owner-operator", "small-shop-owner"]);
  });
  ...
  it("rejects targetPersona with non-string elements", () => {
    expect(() => baseContentSchema.parse({ ...validBase, targetPersona: [123] })).toThrow();
  });
});
...
describe("alternativeSchema", () => {
  ...
  it("accepts tableData with optional description", () => {
    ...
    expect(result.tableData?.description).toBe("Side-by-side pricing breakdown");
  });
  ...
  it("normalizes answers with question/answer format to q/a", () => {
    ...
    expect(result.answers).toEqual([{ q: "What is this?", a: "It is a thing." }]);
  });
      ...
      definitions: [
        {
          ...
          definition: "A platform that collects tee times from multiple courses.",
        },
        {
          term: "P2P exchange",
...
describe("comparisonSchema", () => {
  ...
  it("rejects missing verdict", () => {
    const noVerdict = { ...validComparison };
  ...
  it("accepts pricingStats array", () => {
    ...
    expect(result.pricingStats?.[1].sourceUrl).toBe("https://www.americanbar.org/techreport");
  });

  it("accepts pricingStats entry without sourceUrl", () => {
    const result = comparisonSchema.parse({
      ...validComparison,
      pricingStats: [{ stat: "MyCase starts at $39/user/mo", source: "MyCase.com 2026" }],
    });
...
describe("pricingBreakdownSchema", () => {
  ...
  it("rejects missing tiers", () => {
    const noTiers = { ...validPricing };
    delete noTiers.tiers;
  ...
  it("accepts expertQuotes with all optional fields", () => {
    ...
    expect(result.expertQuotes![0].organization).toBe("Field Service Insights");
  });
...
describe("statePageSchema", () => {
  ...
  it("parses with only state + stateCode (no city overlap fields)", () => {
  ...
  it("parses with new generic fields (marketSize, topMarkets, regulations)", () => {
    const result = statePageSchema.parse({
      ...
      stateCode: "CA",
      marketSize: 25000,
      topMarkets: [{ name: "Los Angeles", count: 5000, label: "HOA communities" }],
      regulations: [
        {
  ...
  it("parses definitions with term and definition fields", () => {
    const result = statePageSchema.parse({
      ...
      definitions: [
        ...
        {
          term: "BAA",
          definition: "Business Associate Agreement — required by HIPAA for covered entities.",
        },
      ],
    });
    expect(result.definitions).toHaveLength(2);
...
describe("cityPageSchema", () => {
  ...
  it("parses minimal city page with required fields", () => {
    const result = cityPageSchema.parse(validCity);
    expect(result.city).toBe("Chicago");
    expect(result.citySlug).toBe("chicago");
    expect(result.state).toBe("Illinois");
    expect(result.stateCode).toBe("IL");
    expect(result.stateSlug).toBe("illinois");
    expect(result.topFunders).toEqual([]);
    expect(result.localRegulations).toEqual([]);
    expect(result.definitions).toEqual([]);
  });

  it("rejects when stateSlug is missing", () => {
    const { stateSlug, ...rest } = validCity;
    void stateSlug;
    expect(() => cityPageSchema.parse(rest)).toThrow();
  });

  it("rejects when citySlug is missing", () => {
    const { citySlug, ...rest } = validCity;
    void citySlug;
    expect(() => cityPageSchema.parse(rest)).toThrow();
  });

  it("parses with topFunders, localRegulations, fiscal/registration notes", () => {
    const result = cityPageSchema.parse({
      ...validCity,
      metroAreaName: "Chicagoland",
      nonprofitCount: 32000,
      topFunders: [
        {
        ...
        { name: "Chicago Community Trust", type: "community-foundation" },
      ],
      localRegulations: [
        {
          heading: "IL AG Charitable Trust",
          content: "Annual AG-990 IL filing",
          variant: "warning",
        },
      ],
      fiscalCalendarNotes: "City of Chicago FY runs Jan 1 - Dec 31.",
      registrationNotes: "IL AG charitable trust registration required.",
    });
    expect(result.topFunders).toHaveLength(2);
    expect(result.topFunders[0].type).toBe("private-foundation");
    expect(result.topFunders[1].annualGiving).toBeUndefined();
    expect(result.localRegulations[0].variant).toBe("warning");
    expect(result.fiscalCalendarNotes).toContain("City of Chicago");
    expect(result.metroAreaName).toBe("Chicagoland");
    expect(result.nonprofitCount).toBe(32000);
  });

  it("rejects unknown funder type", () => {
    expect(() =>
      cityPageSchema.parse({
        ...validCity,
        topFunders: [{ name: "X", type: "made-up-type" }],
      }),
    ).toThrow();
  });

  it("rejects invalid funder url", () => {
    expect(() =>
      cityPageSchema.parse({
        ...validCity,
        topFunders: [{ name: "X", type: "community-foundation", url: "not-a-url" }],
      }),
    ).toThrow();
  });

  it("defaults localRegulation variant to info", () => {
    const result = cityPageSchema.parse({
      ...validCity,
      localRegulations: [{ heading: "Note", content: "..." }],
    });
    expect(result.localRegulations[0].variant).toBe("info");
  });
});
...
describe("verticalPageSchema", () => {
  ...
  it("parses valid vertical page with required fields", () => {
    ...
    expect(result.complianceNotes).toBe("Must file Form 990 if over $200K gross receipts");
  });
  ...
  it("rejects missing verticalType", () => {
    const noType = { ...validVertical };
    delete noType.verticalType;
    expect(() => verticalPageSchema.parse(noType)).toThrow();
  });

  it("rejects missing keyPainPoints", () => {
    const noPainPoints = { ...validVertical };
    delete noPainPoints.keyPainPoints;
    expect(() => verticalPageSchema.parse(noPainPoints)).toThrow();
  });

  it("rejects missing complianceNotes", () => {
    const noNotes = { ...validVertical };
    delete noNotes.complianceNotes;
...
describe("orgTypePageSchema", () => {
  ...
  it("parses valid org-type page with required fields", () => {
    ...
    expect(result.uniqueNeeds).toEqual(["Tithe tracking", "Building fund restrictions"]);
  });
  ...
  it("accepts optional complianceNotes", () => {
    ...
    expect(result.complianceNotes).toBe("Must file Form 990-EZ if under $200K revenue");
  });
  ...
  it("rejects missing orgType", () => {
    const noOrgType = { ...validOrgType };
    delete noOrgType.orgType;
    expect(() => orgTypePageSchema.parse(noOrgType)).toThrow();
  });

  it("rejects missing orgTypeSlug", () => {
    const noSlug = { ...validOrgType };
    delete noSlug.orgTypeSlug;
    expect(() => orgTypePageSchema.parse(noSlug)).toThrow();
  });

  it("rejects missing uniqueNeeds", () => {
    const noNeeds = { ...validOrgType };
    delete noNeeds.uniqueNeeds;
...
describe("reviewSchema", () => {
  const validReview = {
    ...
    verdict: "Clio is overpriced for small firms; CaelusLaw includes IOLTA at no extra cost.",
  };
  ...
  it("rejects missing competitor", () => {
    const noCompetitor = { ...validReview };
    delete noCompetitor.competitor;
    expect(() => reviewSchema.parse(noCompetitor)).toThrow();
  });

  it("rejects missing verdict", () => {
    const noVerdict = { ...validReview };
    delete noVerdict.verdict;
...
describe("featureSchema", () => {
  ...
  it("accepts optional tableData", () => {
    ...
    expect(result.tableData?.name).toBe("Feature Comparison");
    expect(result.tableData?.columns).toHaveLength(3);
    expect(result.tableData?.rows[0]).toEqual(["IOLTA included", "Yes", "Add-on"]);
  });
...
describe("phasePageSchema", () => {
  ...
  it("accepts all valid phase enum values", () => {
    const phases = ["follicular", "ovulatory", "luteal", "menstrual", "hormone", "cycle"] as const;
  ...
  it("rejects empty relatedPages array", () => {
    expect(() => phasePageSchema.parse({ ...validPhase, relatedPages: [] })).toThrow();
  });
  ...
  it("accepts faqs array", () => {
    const result = phasePageSchema.parse({
      ...validPhase,
      faqs: [{ q: "What is the luteal phase?", a: "The phase after ovulation." }],
    });
    expect(result.faqs).toHaveLength(1);
  });
  ...
  it("accepts optional answers", () => {
    const result = phasePageSchema.parse({
      ...validPhase,
      answers: [{ q: "When does the luteal phase start?", a: "After ovulation." }],
    });
    expect(result.answers).toHaveLength(1);
  ...
  it("accepts optional statistics", () => {
    const result = phasePageSchema.parse({
      ...validPhase,
      statistics: [{ stat: "Progesterone peaks in luteal phase", source: "NIH 2023" }],
    });
...
describe("goalPageSchema", () => {
  ...
  it("rejects invalid audience value", () => {
    expect(() => goalPageSchema.parse({ ...validGoal, audience: "men" })).toThrow();
  });
  ...
  it("rejects empty relatedPages array", () => {
    expect(() => goalPageSchema.parse({ ...validGoal, relatedPages: [] })).toThrow();
  });
  ...
  it("accepts faqs array", () => {
    const result = goalPageSchema.parse({
      ...validGoal,
      faqs: [{ q: "What is perimenopause?", a: "Transition before menopause." }],
    });
    expect(result.faqs).toHaveLength(1);
  });
  ...
  it("accepts statisticCitations with sourceUrl", () => {
    ...
    expect(result.statisticCitations![0].sourceUrl).toBe("https://www.mayoclinic.org/menopause");
  });
  ...
  it("accepts tableData with optional description", () => {
    ...
    expect(result.tableData!.description).toBe("Recommended daily intake for women over 40");
  });
...
describe("symptomsSchema", () => {
  ...
  it("parses valid symptom frontmatter", () => {
    const result = symptomsSchema.safeParse({
      ...
      description: "Why brain fog happens in perimenopause and what to do about it.",
...
describe("leadMagnetSchema", () => {
  const validLeadMagnet = {
    ...
    sourceUrls: ["https://www.ecfr.gov/current/title-2/subtitle-A/chapter-II/part-200"],
  };
  ...
  it("rejects empty relatedPages", () => {
    expect(() => leadMagnetSchema.parse({ ...validLeadMagnet, relatedPages: [] })).toThrow();
  });
  ...
  it("requires reviewed and verified source evidence", () => {
    const withoutField = (field: keyof typeof validLeadMagnet): Record<string, unknown> => {
      const copy: Record<string, unknown> = { ...validLeadMagnet };
      delete copy[field];
      return copy;
    };

    expect(() => leadMagnetSchema.parse(withoutField("lastReviewedAt"))).toThrow();
    expect(() => leadMagnetSchema.parse(withoutField("verifiedAt"))).toThrow();
    expect(() => leadMagnetSchema.parse(withoutField("sourceUrls"))).toThrow();
    expect(() => leadMagnetSchema.parse({ ...validLeadMagnet, sourceUrls: [] })).toThrow();
  });
  ...
  it("rejects invalid buyerStage", () => {
    expect(() => leadMagnetSchema.parse({ ...validLeadMagnet, buyerStage: "invalid" })).toThrow();
  });
  ...
  it("rejects invalid schema values", () => {
    expect(() => leadMagnetSchema.parse({ ...validLeadMagnet, schema: "BlogPost" })).toThrow();
  });
  ...
  it("accepts optional seoTitle override", () => {
    const result = leadMagnetSchema.parse({
      ...validLeadMagnet,
      seoTitle: "Free Grant Compliance Checklist | GrantPipe",
    });
    expect(result.seoTitle).toBe("Free Grant Compliance Checklist | GrantPipe");
  });

  it("accepts optional seoDescription override", () => {
    const result = leadMagnetSchema.parse({
      ...validLeadMagnet,
      seoDescription: "Download our free grant compliance checklist for nonprofits.",
    });
    expect(result.seoDescription).toBe(
      "Download our free grant compliance checklist for nonprofits.",
    );
  });
  ...
  it("defaults deliverableType to article when omitted", () => {
    const result = leadMagnetSchema.parse(validLeadMagnet);
    expect(result.deliverableType).toBe("article");
  });

  it("accepts leadMagnetSlug for explicit frontmatter routing on lead magnet pages too", () => {
    const result = leadMagnetSchema.parse({
      ...validLeadMagnet,
      leadMagnetSlug: "grant-compliance-checklist",
    });

    expect(result.leadMagnetSlug).toBe("grant-compliance-checklist");
  });

  it("accepts deliverableType pdf", () => {
    const result = leadMagnetSchema.parse({
      ...validLeadMagnet,
      deliverableType: "pdf",
    });
    expect(result.deliverableType).toBe("pdf");
  });

  it("accepts deliverableType calculator", () => {
    const result = leadMagnetSchema.parse({
      ...validLeadMagnet,
      deliverableType: "calculator",
    });
    expect(result.deliverableType).toBe("calculator");
  });

  it("accepts deliverableType sheet", () => {
    const result = leadMagnetSchema.parse({
      ...validLeadMagnet,
      deliverableType: "sheet",
    });
    expect(result.deliverableType).toBe("sheet");
  });

  it("accepts deliverableType article explicitly", () => {
    const result = leadMagnetSchema.parse({
      ...validLeadMagnet,
  ...
  it("rejects invalid deliverableType", () => {
    expect(() =>
      leadMagnetSchema.parse({ ...validLeadMagnet, deliverableType: "video" }),
    ).toThrow();
  });

  it("accepts an explicit exitMagnetSlug", () => {
    const result = leadMagnetSchema.parse({
      ...validLeadMagnet,
      exitMagnetSlug: "donor-retention-playbook",
    });
    expect(result.exitMagnetSlug).toBe("donor-retention-playbook");
  });

  it("rejects an invalid exitMagnetSlug value", () => {
    expect(() =>
      leadMagnetSchema.parse({ ...validLeadMagnet, exitMagnetSlug: "not-a-real-slug" }),
    ).toThrow();
  });
});
...
describe("personaSchema", () => {
  const validPersona = {
    ...
    roleSlug: "executive-director",
    painPoints: ["Time-poor", "No dedicated ops"],
    jobsToBeDone: ["Report to board", "Track restricted funds"],
    featureMap: [{ feature: "Dashboard", benefit: "See restricted balances at a glance" }],
  };

  it("parses valid persona with required fields", () => {
    const result = personaSchema.parse(validPersona);
    expect(result.role).toBe("Executive Director");
    expect(result.roleSlug).toBe("executive-director");
    expect(result.painPoints).toHaveLength(2);
    expect(result.jobsToBeDone).toHaveLength(2);
    expect(result.featureMap[0].feature).toBe("Dashboard");
    expect(result.schema).toBe("Article");
  });

  it("accepts optional answers, definitions, pricingStats, tableData, proscons, expertQuotes", () => {
    const result = personaSchema.parse({
      ...validPersona,
      answers: [{ q: "Why?", a: "Because." }],
      definitions: [{ term: "Restricted", definition: "Funds with donor restrictions." }],
      pricingStats: [{ stat: "Stat", source: "Source" }],
      tableData: { name: "t", columns: ["a"], rows: [["b"]] },
      proscons: [{ subject: "X", pros: ["p"], cons: ["c"] }],
      expertQuotes: [{ quote: "q", personName: "n" }],
    });
    expect(result.answers).toHaveLength(1);
    expect(result.definitions).toHaveLength(1);
    expect(result.pricingStats).toHaveLength(1);
    expect(result.tableData?.name).toBe("t");
    expect(result.proscons).toHaveLength(1);
    expect(result.expertQuotes).toHaveLength(1);
  });

  it("rejects missing required role", () => {
    const { role: _role, ...rest } = validPersona;
    void _role;
    expect(() => personaSchema.parse(rest)).toThrow();
  });

  it("rejects missing roleSlug", () => {
    const { roleSlug: _s, ...rest } = validPersona;
    void _s;
    expect(() => personaSchema.parse(rest)).toThrow();
  });

  it("rejects missing painPoints", () => {
    const { painPoints: _p, ...rest } = validPersona;
    void _p;
    expect(() => personaSchema.parse(rest)).toThrow();
  });

  it("rejects missing jobsToBeDone", () => {
    const { jobsToBeDone: _j, ...rest } = validPersona;
    void _j;
    expect(() => personaSchema.parse(rest)).toThrow();
  });

  it("rejects missing featureMap", () => {
    const { featureMap: _f, ...rest } = validPersona;
    void _f;
    expect(() => personaSchema.parse(rest)).toThrow();
  });

  it("rejects featureMap items missing benefit", () => {
    expect(() =>
      personaSchema.parse({ ...validPersona, featureMap: [{ feature: "x" }] }),
    ).toThrow();
  });
});
...
describe("workflowSchema", () => {
  const validWorkflow = {
    ...validBase,
    steps: [{ title: "Step 1", content: "Do the thing" }],
    timeEstimate: "30 minutes",
    difficulty: "beginner" as const,
    prerequisites: ["A laptop"],
    outputs: ["A completed grant file"],
  };

  it("parses valid workflow with required fields", () => {
    const result = workflowSchema.parse(validWorkflow);
    expect(result.steps).toHaveLength(1);
    expect(result.timeEstimate).toBe("30 minutes");
    expect(result.difficulty).toBe("beginner");
    expect(result.prerequisites).toEqual(["A laptop"]);
    expect(result.outputs).toEqual(["A completed grant file"]);
    expect(result.schema).toBe("HowTo");
  });

  it("accepts all difficulty levels", () => {
    for (const difficulty of ["beginner", "intermediate", "advanced"]) {
      const result = workflowSchema.parse({ ...validWorkflow, difficulty });
      expect(result.difficulty).toBe(difficulty);
    }
  });

  it("rejects invalid difficulty", () => {
    expect(() => workflowSchema.parse({ ...validWorkflow, difficulty: "expert" })).toThrow();
  });

  it("rejects missing steps", () => {
    const { steps: _s, ...rest } = validWorkflow;
    void _s;
    expect(() => workflowSchema.parse(rest)).toThrow();
  });

  it("rejects missing timeEstimate", () => {
    const { timeEstimate: _t, ...rest } = validWorkflow;
    void _t;
    expect(() => workflowSchema.parse(rest)).toThrow();
  });

  it("rejects missing difficulty", () => {
    const { difficulty: _d, ...rest } = validWorkflow;
    void _d;
    expect(() => workflowSchema.parse(rest)).toThrow();
  });

  it("rejects missing prerequisites", () => {
    const { prerequisites: _p, ...rest } = validWorkflow;
    void _p;
    expect(() => workflowSchema.parse(rest)).toThrow();
  });

  it("rejects missing outputs", () => {
    const { outputs: _o, ...rest } = validWorkflow;
    void _o;
    expect(() => workflowSchema.parse(rest)).toThrow();
  });

  it("rejects step missing content", () => {
    expect(() =>
      workflowSchema.parse({ ...validWorkflow, steps: [{ title: "Only title" }] }),
    ).toThrow();
  });

  it("accepts optional answers, definitions, pricingStats, tableData, expertQuotes", () => {
    const result = workflowSchema.parse({
      ...validWorkflow,
      answers: [{ q: "?", a: "!" }],
      definitions: [{ term: "t", definition: "d" }],
      pricingStats: [{ stat: "s", source: "src" }],
      tableData: { name: "t", columns: ["c"], rows: [["r"]] },
      expertQuotes: [{ quote: "q", personName: "n" }],
    });
    expect(result.answers).toHaveLength(1);
    expect(result.definitions).toHaveLength(1);
...
describe("glossarySchema", () => {
  const validGlossary = {
    ...
    shortDefinition: "A fund whose use is constrained by donor intent.",
    longDefinition:
      "A restricted fund is a portion of an organization's net assets that donors have specified must be used for a particular purpose.",
    relatedTerms: ["Unrestricted Fund", "Temporarily Restricted"],
    examples: ["Scholarship fund", "Building fund"],
  };

  it("parses valid glossary entry with required fields", () => {
    const result = glossarySchema.parse(validGlossary);
    expect(result.term).toBe("Restricted Fund");
    expect(result.shortDefinition).toContain("constrained");
    expect(result.longDefinition).toContain("net assets");
    expect(result.relatedTerms).toEqual(["Unrestricted Fund", "Temporarily Restricted"]);
    expect(result.examples).toHaveLength(2);
    expect(result.schema).toBe("Article");
  });

  it("rejects missing term", () => {
    const { term: _t, ...rest } = validGlossary;
    void _t;
    expect(() => glossarySchema.parse(rest)).toThrow();
  });

  it("rejects missing shortDefinition", () => {
    const { shortDefinition: _s, ...rest } = validGlossary;
    void _s;
    expect(() => glossarySchema.parse(rest)).toThrow();
  });

  it("rejects missing longDefinition", () => {
    const { longDefinition: _l, ...rest } = validGlossary;
    void _l;
    expect(() => glossarySchema.parse(rest)).toThrow();
  });

  it("rejects missing relatedTerms", () => {
    const { relatedTerms: _r, ...rest } = validGlossary;
    void _r;
    expect(() => glossarySchema.parse(rest)).toThrow();
  });

  it("rejects missing examples", () => {
    const { examples: _e, ...rest } = validGlossary;
    void _e;
    expect(() => glossarySchema.parse(rest)).toThrow();
  });

  it("accepts optional answers, definitions, pricingStats, expertQuotes", () => {
    const result = glossarySchema.parse({
      ...validGlossary,
      answers: [{ q: "?", a: "!" }],
      definitions: [{ term: "t", definition: "d" }],
      pricingStats: [{ stat: "s", source: "src" }],
  ...
  it("accepts SoftwareApplication and DefinedTerm schema values on baseContentSchema", () => {
    const app = baseContentSchema.parse({ ...validBase, schema: "SoftwareApplication" });
    expect(app.schema).toBe("SoftwareApplication");
    const term = baseContentSchema.parse({ ...validBase, schema: "DefinedTerm" });
    expect(term.schema).toBe("DefinedTerm");
  });
});
...
describe("integrationSchema", () => {
  const validIntegration = {
    ...validBase,
    partner: { name: "QuickBooks Online", slug: "quickbooks-online" },
    category: "accounting" as const,
    setupSteps: [{ title: "Connect", content: "Authenticate via OAuth." }],
    supportedFeatures: ["Chart of accounts sync"],
    useCases: ["Reconcile grant revenue monthly"],
  };

  it("parses valid integration with required fields and SoftwareApplication default", () => {
    const result = integrationSchema.parse(validIntegration);
    expect(result.partner.name).toBe("QuickBooks Online");
    expect(result.partner.slug).toBe("quickbooks-online");
    expect(result.category).toBe("accounting");
    expect(result.setupSteps).toHaveLength(1);
    expect(result.supportedFeatures).toEqual(["Chart of accounts sync"]);
    expect(result.useCases).toEqual(["Reconcile grant revenue monthly"]);
    expect(result.schema).toBe("SoftwareApplication");
  });

  it("accepts optional partner url and logo", () => {
    const result = integrationSchema.parse({
      ...
      partner: {
        ...
        logo: "/images/logos/qbo.svg",
      },
    });
    expect(result.partner.url).toBe("https://quickbooks.intuit.com");
    expect(result.partner.logo).toBe("/images/logos/qbo.svg");
  });

  it("accepts all category values", () => {
    for (const category of ["accounting", "email", "payments", "automation", "other"]) {
      const result = integrationSchema.parse({ ...validIntegration, category });
      expect(result.category).toBe(category);
    }
  });

  it("rejects invalid category", () => {
    expect(() => integrationSchema.parse({ ...validIntegration, category: "invalid" })).toThrow();
  });

  it("rejects missing partner", () => {
    const { partner: _p, ...rest } = validIntegration;
    void _p;
    expect(() => integrationSchema.parse(rest)).toThrow();
  });

  it("rejects partner missing name or slug", () => {
    expect(() =>
      integrationSchema.parse({ ...validIntegration, partner: { name: "x" } }),
    ).toThrow();
    expect(() =>
      integrationSchema.parse({ ...validIntegration, partner: { slug: "x" } }),
    ).toThrow();
  });

  it("rejects missing setupSteps", () => {
    const { setupSteps: _s, ...rest } = validIntegration;
    void _s;
    expect(() => integrationSchema.parse(rest)).toThrow();
  });

  it("rejects setup step missing content", () => {
    expect(() =>
      integrationSchema.parse({
        ...validIntegration,
        setupSteps: [{ title: "Only title" }],
      }),
    ).toThrow();
  });

  it("rejects missing supportedFeatures", () => {
    const { supportedFeatures: _f, ...rest } = validIntegration;
    void _f;
    expect(() => integrationSchema.parse(rest)).toThrow();
  });

  it("rejects missing useCases", () => {
    const { useCases: _u, ...rest } = validIntegration;
    void _u;
    expect(() => integrationSchema.parse(rest)).toThrow();
  });

  it("accepts optional tableData, answers, pricingStats, proscons, expertQuotes", () => {
    const result = integrationSchema.parse({
      ...validIntegration,
      tableData: { name: "t", columns: ["c"], rows: [["r"]] },
      answers: [{ q: "?", a: "!" }],
      pricingStats: [{ stat: "s", source: "src" }],
      proscons: [{ subject: "x", pros: ["a"], cons: ["b"] }],
      expertQuotes: [{ quote: "q", personName: "n" }],
    });
    expect(result.tableData?.name).toBe("t");
    expect(result.answers).toHaveLength(1);
  ...
  it("rejects missing required base fields", () => {
    expect(() => integrationSchema.parse({})).toThrow();
  });
});
